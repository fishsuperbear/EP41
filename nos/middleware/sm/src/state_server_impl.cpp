...
std::string StateServerImpl::GetProcessName()
{
    return "StateServerImpl";
}

int32_t StateServerImpl::Start()
{
    SM_SERVER_LOG_INFO << "state_cm_server.RegisterProcess and Start with topic state_client_request";
    state_cm_server.Start(0, "state_client_request");
    return 0;
}

void StateServerImpl::Stop()
{
    state_cm_server.Stop();
}

bool StateServerImpl::PreAndPostProcess(const ProcessMode mod, const std::string& src_mode, const std::string& tar_mode, std::map<string, string>& map)
{
    bool ret = true;
    if (map.size() == 0) {
...
int32_t StateServerImpl::SendRequestToClient(const uint32_t& type, const uint32_t& reply_type, const std::string& old_mode, const std::string& new_mode, const bool& succ, const string& client_process_name, const string& extra_data, const bool& forget)
{
    SM_SERVER_LOG_INFO << "SendRequestToClient: type = " << FormatType(type) << ", old_mode is [" << old_mode << "], new_mode is [" << new_mode << "],extra_data is [" << extra_data << "]";

    std::string topic_name = client_process_name + TOPIC_SUFFIX_NAME;
    SM_SERVER_LOG_INFO << "topic is [" + topic_name + "]";
    client.Init(0, topic_name);
    int online = client.WaitServiceOnline(500);  //用户需要去调等待服务
    SM_SERVER_LOG_INFO << "online: " << online;

    req_data->type(type);
    req_data->process_name(client_process_name);
    ...
    if (forget) {
        req_data->fire_forget(true);
        res = client.RequestAndForget(req_data);
        SM_SERVER_LOG_INFO << "SendRequestToClient: client.RequestAndForget return value res is [" << res << "]";
        if (res == 0) {
            res = static_cast<int32_t>(SmResultCode::kSuccess);
        } else {
            res = static_cast<int32_t>(SmResultCode::kFailed);
        }
    } else {
        req_data->fire_forget(false);
        res = client.Request(req_data, resp_data, 5000*5);
        if (res == 0) {
            SM_SERVER_LOG_INFO << "SendRequestToClient: client.Request return value is OK, res is [" << res << "]; resp process name is [" << resp_data->process_name()
                        << "], callback name is " << req_data->extra_data();
            if (resp_data->type() == reply_type) {
                if (resp_data->result() == 0) {
                    SM_SERVER_LOG_INFO << "Request " << FormatType(type) << ", reply type " << FormatType(resp_data->type()) << ", is OK, and result[" << resp_data->result() << "] is success";
                    res = static_cast<int32_t>(SmResultCode::kSuccess);
                } else {
                    SM_SERVER_LOG_ERROR << "Request " << FormatType(type) << ", reply type " << FormatType(resp_data->type()) << ", is OK, but result[" << resp_data->result() << "] is fail";
                    res = resp_data->result();
                }
            } else {
                SM_SERVER_LOG_INFO << "Request " << FormatType(type) << ", reply type " << FormatType(resp_data->type()) << ", is OK";
                res = static_cast<int32_t>(SmResultCode::kInvalid);
            }
        } else {
            SM_SERVER_LOG_INFO << "SendRequestToClient: client.Request type =" << FormatType(req_data->type()) << "; return value is NG, res is ["
                        << res << "]; resp type is " << FormatType(resp_data->type()) << ",; resp process name is [" << resp_data->process_name() << "], callback name is " << req_data->extra_data();
            res = static_cast<int32_t>(SmResultCode::kTimeout);
        }
    }
...
int32_t StateServerImpl::RequestProcess(const std::shared_ptr<sm_request> req, std::shared_ptr<sm_reply> resp)
{
    SM_SERVER_LOG_DEBUG << "StateServerImpl::RequestProcess received";
    ...
    switch(req->type()) {
        case REQUEST_CODE_REGISTER_PREPROCESS_FUNC:
        {
            ...
            resp->type(REPLY_CODE_REGISTER_PREPROCESS_FUNC);
            resp->process_name(GetProcessName());
            resp->result(0);
            SM_SERVER_LOG_INFO << "resp->type is " << FormatType(resp->type()) << ", resp->process_name() is [" << resp->process_name() << "], res is [" << resp->result() << "]";
            break;
        }
        case REQUEST_CODE_REGISTER_POSTPROCESS_FUNC:
        {
            ...
            resp->type(REPLY_CODE_REGISTER_POSTPROCESS_FUNC);
            resp->process_name(GetProcessName());
            resp->result(0);
            SM_SERVER_LOG_INFO << "resp->type is " << FormatType(resp->type()) << ", resp->process_name() is [" << resp->process_name() << "], res is [" << resp->result() << "]";
            break;
        ...
        case REQUEST_CODE_SWITCH_MODE:
        {
            ...
            string process_name = req->process_name();
            string old_mode = ExecManagement::Instance()->GetSysMode();
            string new_mode = req->new_mode();
            SM_SERVER_LOG_INFO << "Request: process name is [" << process_name << "]; old_mode is [" << old_mode <<  "]; new_mode is [" << new_mode << "]";

            /* PreProcess */
            map<string, string> pre_trans_copy;
            {
                std::lock_guard<std::mutex> lock(preprocess_funcmap_mutex);
                SM_SERVER_LOG_INFO << "PreProcessFuncMap size is [" << PreProcessFuncMap.size() << "]";
            ...
            /* PostProcess */
            map<string, string> post_trans_copy;
            {
                std::lock_guard<std::mutex> lock(postprocess_funcmap_mutex);
                SM_SERVER_LOG_INFO << "PostProcessFuncMap size is [" << PostProcessFuncMap.size() << "]";
            ...
            resp->type(REPLY_CODE_SWITCH_MODE);
            resp->process_name(GetProcessName());
            resp->result(switchmode_result);
            SM_SERVER_LOG_INFO << "resp->type is " << FormatType(resp->type()) << ", resp->process_name() is [" << resp->process_name() << "], res is [" << resp->result() << "]";
            break;
        }
        case REQUEST_CODE_STOP_MODE:
        {
            ...
            resp->type(REPLY_CODE_STOP_MODE);
            resp->process_name(GetProcessName());
            resp->result(res);
            SM_SERVER_LOG_INFO << "resp->type is " << FormatType(resp->type()) << ", resp->process_name() is [" << resp->process_name() << "], res is [" << resp->result() << "]";
            break;
        ...
        case REQUEST_CODE_GET_CURR_MODE:
        {
            ...
            resp->type(REPLY_CODE_GET_CURR_MODE);
            resp->process_name(GetProcessName());

            std::string curr_mode = ExecManagement::Instance()->GetSysMode();
            SM_SERVER_LOG_DEBUG << "GetCurrMode Request, curr_mode is [" << curr_mode << "]";
            resp->curr_mode(curr_mode);
            resp->result(0);
            SM_SERVER_LOG_DEBUG << "resp->type is " << FormatType(resp->type()) << ", resp->process_name() is [" << resp->process_name() << "], res is [" << resp->result() << "]";
            break;
        }
        case REQUEST_CODE_SET_DEFAULT_MODE:
        {
            ...
            SM_SERVER_LOG_INFO << "ExecManagement::Instance()->SetDefaultMode to mode [" << req->new_mode() << "], res is [" << resp->result() << "]";

            resp->type(REPLY_CODE_SET_DEFAULT_MODE);
            resp->process_name(GetProcessName());
            resp->result(res);
            SM_SERVER_LOG_INFO << "resp->type is " << FormatType(resp->type()) << ", resp->process_name() is [" << resp->process_name() << "], res is [" << resp->result() << "]";
            break;
        ...
        case REQUEST_CODE_GET_PROC_INFO:
        {
            ...
            if (res == 0) {
                ...
                for (auto proc_info: proc_info_data) {
                    ...
                    process_info proc_info_temp;
                    proc_info_temp.group(proc_info.group);
                    proc_info_temp.procname(proc_info.procname);
                    proc_info_temp.procstate(static_cast<uint32_t>(proc_info.procstate));
                    reply_data.emplace_back(proc_info_temp);
                }
            }
            resp->data(reply_data);
            resp->result(res);
        ...
        case REQUEST_CODE_PROC_RESTART:
        {
            ...
            resp->type(REPLY_CODE_PROC_RESTART);
            resp->process_name(GetProcessName());
        ...
        case REQUEST_CODE_GET_MODE_LIST:
        {
            ...
            resp->type(REPLY_CODE_GET_MODE_LIST);
            resp->process_name(GetProcessName());
