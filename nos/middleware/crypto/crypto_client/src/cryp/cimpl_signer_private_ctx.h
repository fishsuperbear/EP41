...
namespace crypto {
namespace cryp {
class CimplSignerPrivateCtx : public SignerPrivateCtx{
public:
    // struct OpensslCtx
    // {
    ...
    using Uptr = std::unique_ptr<CimplSignerPrivateCtx>;
    CimplSignerPrivateCtx(AlgId id):alg_id_(id){}
    CimplSignerPrivateCtx(const CipherCtxRef& ctx_ref);
    ~CimplSignerPrivateCtx();
    ...
    CipherCtxRef getSignerPrivateCtx() {
        return ctx_ref_;
    }
private:
