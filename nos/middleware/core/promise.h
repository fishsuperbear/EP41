#ifndef NETAOS_CORE_PROMISE_H
#define NETAOS_CORE_PROMISE_H
...
namespace core {
/**
 * @brief netaos::core specific variant of std::promise class [SWS_CORE_00340].
 *
 * @tpnetaosm T             the type of value
 * @tpnetaosm ErrorCode     the type of error
 */
...
class Promise {
   ...
   public:
     ...
     *
     * @pnetaosm other  the other instance
     */
    Promise(Promise&& other) noexcept : lock_(other.mutex_), stdPromise_(std::move(other.stdPromise_)), state_(std::move(other.state_)) { lock_.unlock(); }
     ...
     */
    Future<T, E> get_future() { return Future<T, E>(stdPromise_.get_future(), state_); }

    /* *
     * @brief Move an error into the shared state and make the state ready [SWS_CORE_00353].
     *
     * @pnetaosm[in] error the error to store
     */
     ...
     *
     * @pnetaosm[in] value the value to store
     */
...
class Promise<void, E> final {
   ...
   public:
     ...
     */
    Promise(Promise&& other) noexcept : lock_(other.mutex_), stdPromiseSpec_(std::move(other.stdPromiseSpec_)), state_(std::move(other.state_)) { lock_.unlock(); }
     ...
     */
    Future<void, E> get_future() { return Future<void, E>(stdPromiseSpec_.get_future(), state_); }

    /* *
     * @brief Move an error into the shared state and make the state ready [SWS_CORE_06353].
     *
     * @pnetaosm[in] err  the error to store
     */
    void SetError(E&& err) {
