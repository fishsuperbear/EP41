#ifndef NETAOS_CORE_VARIANT_UTIL_H
#define NETAOS_CORE_VARIANT_UTIL_H
#include <limits>
#include <type_traits>
namespace hozon {
...
enum : size_t { NotFound = std::numeric_limits<size_t>::max(), Ambiguous = NotFound - 1 };
...
template <size_t idx>
struct FindUnambiguousIndexSfinaeImpl : std::integral_constant<size_t, idx> {};
...
template <typename T, typename... Types>
struct FindUnambiguousIndexSfinae : FindUnambiguousIndexSfinaeImpl<FindIndex<T, Types...>()> {};

inline constexpr size_t FindIdxReturn(size_t currIdx, size_t res, bool match) { return !match ? res : (res == NotFound ? currIdx : Ambiguous); }
...
struct ImaginaryFun<hozon::netaos::core::Variant<>, I> {
    static void Run() = delete;
};

template <typename T, typename... Types, size_t I>
struct ImaginaryFun<hozon::netaos::core::Variant<T, Types...>, I> : ImaginaryFun<hozon::netaos::core::Variant<Types...>, I + 1> {
    using ImaginaryFun<hozon::netaos::core::Variant<Types...>, I + 1>::Run;
...
struct IndexOfConstructedType<Variant, T, hozon::netaos::core::internal::typeTraits::void_t<decltype(ImaginaryFun<Variant>::Run(std::declval<T>(), {}))>>
...
struct GetTypeIndex {
    enum : size_t { index = GetTypeIndex<N + 1, T, Types...>::index };
};
