#ifndef NETAOS_CORE_RESULT_H
#define NETAOS_CORE_RESULT_H
#include <memory>
...
class Result final {
   public:
    ...
    Result(Result&& other) noexcept(std::is_nothrow_move_constructible<T>::value&& std::is_nothrow_move_constructible<E>::value) : error_(GetDefaultErrorCode()) {
        valueFlag_ = other.valueFlag_;
        if (valueFlag_ && (other.value_ != nullptr)) {
    ...
    template <typename... Args,
              typename = typename std::enable_if<std::is_constructible<T, Args&&...>::value && !internal::IsFirstType<T, Args...>::value && !internal::IsFirstType<Result, Args...>::value>::type>
    ...
    template <typename... Args,
              typename = typename std::enable_if<std::is_constructible<E, Args...>::value && !internal::IsFirstType<E, Args...>::value && !internal::IsFirstType<Result, Args...>::value>::type>
    ...
    Result& operator=(Result&& other) noexcept(
        std::is_nothrow_move_constructible<T>::value&& std::is_nothrow_move_assignable<T>::value&& std::is_nothrow_move_constructible<E>::value&& std::is_nothrow_move_assignable<E>::value) {
        if (this == &other) {
            return *this;
        }
        valueFlag_ = other.valueFlag_;
    ...
    bool HasValue() const noexcept { return valueFlag_ && (value_ != nullptr); }
    ...
    T const& Value() const& {
        ...
        hozon::netaos::core::Abort("[CORETYPE Result] : Error, Value() called but NOT a value!");
        // The std::abort Will never be executed, for process will be terminated in hozon::netaos::core::Abort(), And std::abort
    ...
    E const& Error() const& {
        ...
        hozon::netaos::core::Abort("[CORETYPE Result] : Error, Error() called but NOT a error!");
        // The std::abort Will never be executed, for process will be terminated in hozon::netaos::core::Abort(), And std::abort
        // is used to prevent warnings when compilation.
        std::abort();
    }
...
class Result<void, E> final {
   public:
    ...
    Result() noexcept
        : valueFlag_(true),
          error_(GetDefaultErrorCode())  // [SWS_CORE_00821]
    {}

    explicit Result(E const& e)
        : valueFlag_(false),
          error_(e)  // [SWS_CORE_00823]
    {}

    explicit Result(E&& e)
        : valueFlag_(false),
          error_(std::move(e))  // [SWS_CORE_00824]
    {}
    ...
    void Swap(Result& other) noexcept(std::is_nothrow_move_constructible<E>::value&& std::is_nothrow_move_assignable<E>::value)  // [SWS_CORE_00845]
    {
        std::swap(error_, other.error_);
        std::swap(valueFlag_, other.valueFlag_);
    }
