...
DiagServerTransportMethodSender::Init()
{
    ...
    if (bResult) {
        for (auto& item: service) {
            ...
            {
                DG_INFO << "item 1" << item;
                diag_update_client_ = std::make_shared<IPCClient>();
                diag_update_client_->Init(item);
            }
            else
            {
    ...
    update_status_client_ = std::make_shared<IPCClient>();
    update_status_client_->Init("update_status");
}
...
DiagServerTransportMethodSender::DeInit()
{
    ...
    if (nullptr != diag_update_client_) {
        diag_update_client_->Deinit();
        diag_update_client_ = nullptr;
    }
}
...
DiagServerTransportMethodSender::DiagMethodSend(const uint8_t sid, const uint8_t subFunc, const std::vector<std::string> service, std::vector<uint8_t>& udsData)
{
    ...
    std::uint32_t pos_sid{0};
    std::uint32_t pos_subid{0};
    std::vector<uint8_t> pos_data{};
    std::vector<std::string> new_service = service;

    if (nullptr == diag_update_client_ && client_map_.empty()) 
    {
        DG_ERROR << "DiagServerTransportMethodSender::DiagMethodSend client map is empty.";
        return;
    }

    auto it = std::find(service.begin(), service.end(), "diag_update");
    if (it != service.end()) {
        // 删除diag_update
        auto newEnd = std::remove(new_service.begin(), new_service.end(), "diag_update");
        new_service.erase(newEnd, new_service.end());
        ...
        reqProtoData.set_sid(sid);
        reqProtoData.set_subid(subFunc);
        std::vector<uint8_t> dataVec{};
        dataVec.assign(udsData.begin(), udsData.end());
        reqProtoData.set_data_len(dataVec.size());
        reqProtoData.set_data_vec(dataVec.data(), dataVec.size());
        dataVec.clear();
        std::string serializedData = reqProtoData.SerializeAsString();
        req.assign(serializedData.begin(), serializedData.end());
        // Request
        udsData.clear();

        // if cm method not connect, wait connect(timeout: 5s)
        for (int i  = 0; i < 100; i++) {
            if (0 == diag_update_client_->IsMatched()) {
                break;
            }
        ...
        int32_t iResult = diag_update_client_->Request(req, resp, REQUEST_TIMEOUT);

        if (0 != iResult) {
            DG_ERROR << "DiagServerTransportMethodSender::DiagMethodSend service name: diag_update request failed.";
        ...
        respProtoData.ParseFromString(str);
        respVec.assign(respProtoData.data_vec().begin(), respProtoData.data_vec().end());

        if (respProtoData.resp_ack()) {
            udsData.push_back(DiagServerNrcErrc::kNegativeHead);
            udsData.push_back(respProtoData.sid());

            for (auto& item : respVec) {
                udsData.push_back(item);
            }

            return;
        } else {
            pos_sid = respProtoData.sid() + 0x40;
            pos_subid = respProtoData.subid();

            for (auto& item : respVec) {
                pos_data.push_back(item);
            }
        }
    } else {
        DG_ERROR << "DiagServerTransportMethodSender::DiagMethodSend service name: diag_update client is nullptr.";
    }

    // 除了diag_update，其他服务走CM
    if (new_service.size() > 0)
    {
        std::shared_ptr<uds_data_method> req_data = std::make_shared<uds_data_method>();
        req_data->sid(sid);
        req_data->subid(subFunc);
        std::vector<uint8_t> dataVect{};
        dataVect.assign(udsData.begin(), udsData.end());
        req_data->data_len(dataVect.size());
        req_data->data_vec(dataVect);

        // Request
        req_data->fire_forget(false);
        udsData.clear();
        for (uint i = 0; i < new_service.size(); i++) {
            DG_DEBUG << "DiagServerTransPortCM::DiagMethodSend service name: " << new_service[i];
            auto itr = client_map_.find(new_service[i]);
            if (itr == client_map_.end()) {
                DG_WARN << "DiagServerTransportMethodSender::DiagMethodSend error service name: " << new_service[i];
                continue;
            }

            if (nullptr == itr->second) {
                DG_ERROR << "DiagServerTransportMethodSender::DiagMethodSend service name: " << new_service[i] << "client is nullptr.";
                continue;
            }
            ...
            if (0 != iResult) {
                DG_ERROR << "DiagServerTransportMethodSender::DiagMethodSend service name: " << new_service[i] << " request failed.";
                udsData.push_back(DiagServerNrcErrc::kNegativeHead);
                udsData.push_back(sid);
                udsData.push_back(DiagServerNrcErrc::kConditionsNotCorrect);
                return;
            ...
            if (i == (new_service.size() - 1)) {
                pos_sid = resq_data->sid() + 0x40;
                pos_subid = resq_data->subid();
                for (auto& item : resq_data->data_vec()) {
                    pos_data.push_back(item);
                }
            }
    ...
    // 将上面记录的正响应数据返回
    udsData.push_back(pos_sid);
    if (0xFF != pos_subid) {
        udsData.push_back(pos_subid);
    }
...
DiagServerTransportMethodSender::IsCheckUpdateStatusOk()
{
    ...
    std::vector<uint8_t> req{};
    std::vector<uint8_t> resp{};
    req.push_back(static_cast<uint8_t>(DiagUpdateStatus::kDefault));

    // if cm method not connect, wait connect(timeout: 5s)
    for (int i  = 0; i < 100; i++) {
        if (0 == update_status_client_->IsMatched()) {
            break;
        }
    ...
    int iResult = update_status_client_->Request(req, resp, 50);
    if (0 != iResult) {
    ...
    if (resp.size() != 1)
    {
        DG_ERROR << "DiagServerTransportMethodSender::IsCheckUpdateStatusOk data error. size is :" << resp.size();
        return false;
    }

    DiagUpdateStatus status = static_cast<DiagUpdateStatus>(resp.at(0));
    DG_INFO << "DiagServerTransportMethodSender::IsCheckUpdateStatusOk update_status: " << status;
    if (DiagUpdateStatus::kUpdating == status) {
        return false;
    }
