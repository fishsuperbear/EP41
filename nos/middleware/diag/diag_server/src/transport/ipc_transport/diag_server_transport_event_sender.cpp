...
DiagServerTransportEventSender::Init()
{
    ...
    // diag skeleton init
    diag_skeleton_ = std::make_shared<IPCClient>();
    diag_skeleton_->Init("uds_raw_data_resp_eventTopic");
...
DiagServerTransportEventSender::DiagEventSend(const DiagServerRespUdsMessage& udsMessage)
{
    ...
    UdsRawDataRespMethod respProtoData{};

    respProtoData.set_sa(udsMessage.udsSa);
    respProtoData.set_ta(udsMessage.udsTa);
    respProtoData.set_bus_type(static_cast<uint32_t>(udsMessage.busType)); 
    respProtoData.set_result(udsMessage.result);
    respProtoData.set_data_vec(udsMessage.udsData.data(), udsMessage.udsData.size());
    std::vector<uint8_t> vecProto = std::vector<uint8_t>(respProtoData.data_vec().begin(), respProtoData.data_vec().end());


    std::string data = respProtoData.SerializeAsString();
    std::vector<uint8_t> req{};
    req.assign(data.begin(), data.end());

    if (diag_skeleton_->IsMatched() == 0) {
        if (0 == diag_skeleton_->RequestAndForget(req)) {
            DG_INFO << "DiagServerTransportEventSender::DiagEventSend -> sa: " << UINT16_TO_STRING(respProtoData.sa()) \
                    << " ta: " << UINT16_TO_STRING(respProtoData.ta()) << " bus_type: " << respProtoData.bus_type() \
                    << " result: " << respProtoData.result() << " data_vec: " << UINT8_VEC_TO_STRING(vecProto);
        }
    }
}
