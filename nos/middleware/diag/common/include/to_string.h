...
template <class T>
static std::string ToString(T t[], std::ios_base & (*f)(std::ios_base&), uint num = 0, int n = 0, uint pnum = PRINT_DATA_MAX_NUMBER)
{
    if ((nullptr == t) || (0 == num)) {
        return "";
    }
...
template <class T>
static std::string ToString(std::vector<T> t, std::ios_base & (*f)(std::ios_base&), int n = 0, uint pnum = PRINT_DATA_MAX_NUMBER)
{
    if (t.size() <= 0) {
        return "";
    }
...
#define UINT8_VEC_TO_STRING_P(vec, pnum) ToString<uint8_t>(vec, std::hex, 2, pnum)
#define UINT16_VEC_TO_STRING(vec) ToString<uint16_t>(vec, std::hex, 4)
#define UINT16_VEC_TO_STRING_P(vec, pnum) ToString<uint16_t>(vec, std::hex, 4, pnum)
#define UINT32_VEC_TO_STRING(vec) ToString<uint32_t>(vec, std::hex, 8)
#define UINT32_VEC_TO_STRING_P(vec, pnum) ToString<uint32_t>(vec, std::hex, 8, pnum)

#define CHAR_ARRAY_TO_STRING(array, num) ToString<char>(array, std::hex, num, 2)
#define CHAR_ARRAY_TO_STRING_P(array, num, pnum) ToString<char>(array, std::hex, num, 2, pnum)
