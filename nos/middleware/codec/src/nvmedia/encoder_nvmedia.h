 ...
 * Licensed Hozon
 ******************************************************************************/
#ifndef CODEC_ENCODER_NVMEDIA_H_
#define CODEC_ENCODER_NVMEDIA_H_
#pragma once

#include "codec/include/encoder.h"
#include "codec/src/nvmedia/iep/image_encoder_api.h"
...
class EncoderNvmedia : public Encoder {
 public:
  EncoderNvmedia() = default;
  ~EncoderNvmedia();
  /**
   * @brief 初始化decode
   *
  ...
  CodecErrc Process(const void *in, std::vector<std::uint8_t>& out_buff, FrameType& frame_type);
  CodecErrc Process(const void *in, std::string& out_buff, FrameType& frame_type);
