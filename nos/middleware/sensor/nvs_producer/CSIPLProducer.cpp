...
uint64_t TscToRealtimeUs(uint64_t tsc) {
...
uint64_t TscToPTPtimeUs(uint64_t tsc) {
    uint64_t framecapturetsc = tsc * 32;
    uint64_t framecapturetscptp = 0;
    int pps_fd = open("/dev/nvpps0", O_RDONLY);
    struct nvpps_timeevent event;

    if (ioctl(pps_fd, NVPPS_GETEVENT, &event) >= 0) {
        framecapturetscptp = framecapturetsc - event.tsc + event.ptp;
    } else {
        LOG_ERR("ioctl: %s\n", strerror(errno));
    }
    ...
    return framecapturetscptp / 1000;
}
...
CSIPLProducer::MapPayload(INvSIPLClient::ConsumerDesc::OutputType outputType, void *pBuffer, uint32_t &packetIndex)
{
    ...
    if (m_metaPtrs[packetIndex] != nullptr) {
        ...
        // uint64_t captureImgTimestamp = md.frameCaptureStartTSC*32/1000;
        // captureImgTimestamp = ChangeMonoTimeToPtpTime(captureImgTimestamp);
        #if 0
        LOG_MSG("TimeTest sensor_%02d: tsctime:%ld   captureTime:%ld   globalTime:%ld   ptpTime:%ld\n",
            m_uSensorId,md.frameCaptureTSC,(uint64_t)md.frameCaptureTSC*32/1000,md.captureGlobalTimeStamp,captureImgTimestamp);
        #endif
        static_cast<MetaData *>(m_metaPtrs[packetIndex])->captureImgTimestamp = TscToPTPtimeUs(md.frameCaptureStartTSC);
        static_cast<MetaData *>(m_metaPtrs[packetIndex])->streamImageCount = sendImgCount;
    }

    if(sendImgCount % 10 == 0) {
        LOG_INFO("Camera sensor id [%d] image frame index [%d] timestamp [%ld] \n",m_uSensorId, sendImgCount, 
            static_cast<MetaData *>(m_metaPtrs[packetIndex])->captureImgTimestamp);
    }
