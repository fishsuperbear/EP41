...
void ImageBase::read() {
  ...
  if (!infile.is_open()) {
    std::cerr << "ERROR: cannot open PPM image file: " << mPPM.filename
              << std::endl;
  }
...
void ImageBase::write() {
  ...
  outfile << mPPM.magic << " " << mPPM.w << " " << mPPM.h << " " << mPPM.max
          << std::endl;
  outfile.write(reinterpret_cast<char*>(mPPM.buffer.data()), volume());
  outfile.close();
}

RGBImageReader::RGBImageReader(const std::string& filename,
  ...
  if (mPPM.h == H && mPPM.w == W) {
    for (int c = 0; c < C; c++) {
      for (int j = 0, HW = H * W; j < HW; ++j) {
        buffer.get()[c * HW + j] =
            (static_cast<float>(mPPM.buffer[j * C + c]) / mPPM.max - mMean[c]) /
            mStd[c];
      }
    }
  } else {
...
ArgmaxImageWriter::ArgmaxImageWriter(const std::string& filename,
                                     const nvinfer1::Dims& dims,
