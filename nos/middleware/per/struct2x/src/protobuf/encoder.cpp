...
size_t BufferWrapper::size() const {
    size_t temp = (_target - reinterpret_cast<uint8_t*>(&*_buffer->begin()));
    return temp;
}

std::string* BufferWrapper::buffer() { return _buffer; }

uint8_t*& BufferWrapper::target() { return _target; }
...
void BufferWrapper::reserve(uint32_t nSize) {
    ...
    _target = reinterpret_cast<uint8_t*>(&*_buffer->begin());
}
...
bool BufferWrapper::isGetLength() const { return _bCalculateFlag; }
...
void PBEncoder::encodeField(const serializeItem<float>& value) { _mgr->bindValue(&PBEncoder::encodeValue, value); }

void PBEncoder::encodeField(const serializeItem<double>& value) { _mgr->bindValue(&PBEncoder::encodeValue, value); }

void PBEncoder::encodeField(const serializeItem<std::string>& value) { _mgr->bindValue(&PBEncoder::encodeValue, value); }
...
void PBEncoder::svarInt(uint32_t value, proto::BufferWrapper& buf) { varInt((value << 1) ^ static_cast<uint32_t>(value >> 31), buf); }

void PBEncoder::svarInt(uint64_t value, proto::BufferWrapper& buf) { varInt((value << 1) ^ static_cast<uint64_t>(value >> 63), buf); }
