...
void Message::call(uint32_t field_number, void* pStruct, const void* cValue) const {
    ...
    if (it != _functionSet.end()) {
        it->second(reinterpret_cast<uint8_t*>(pStruct), cValue);
    }
}
...
bool Message::ReadVarInt(const uint8_t*& current, size_t& remaining, uint64_t& result) {
    ...
    do {
        ...
        if (!ReadFromBytes(current, remaining, next_number)) return false;
...
bool Message::ParseFromBytes(const uint8_t* sz, uint32_t size, void* pStruct) {
    ...
    while (remaining > 0) {
        ...
        if (!ReadWirteTypeAndFieldNumber(current, remaining, wirte_type, field_number)) return false;
        switch (wirte_type) {
            case struct2x::internal::WT_VARINT: {
                uint64_t value = 0;
                if (!ReadVarInt(current, remaining, value)) return false;
                call(field_number, pStruct, &value);
            } break;
            case struct2x::internal::WT_64BIT: {
                uint64_t value = 0;
                if (!ReadFromBytes(current, remaining, value)) return false;
                call(field_number, pStruct, &value);
            } break;
            case struct2x::internal::WT_LENGTH_DELIMITED: {
                uint64_t size = 0;
                if (!ReadVarInt(current, remaining, size)) return false;
...
PBDecoder::PBDecoder(const uint8_t* sz, unsigned int size) : _msg(NULL), _sz(sz), _size(size), _bParseResult(true) {}

PBDecoder::PBDecoder(const std::string& str) : _msg(NULL), _sz((const uint8_t*)str.c_str()), _size(str.size()), _bParseResult(true) {}

bool PBDecoder::decodeValue(serializeItem<bool>& v) { return _msg->bind<uint64_t, bool>(&PBDecoder::convertValue, v); }

bool PBDecoder::decodeValue(serializeItem<int32_t>& v) { return _msg->bind<uint64_t, int32_t>(&PBDecoder::convertValue, v); }

bool PBDecoder::decodeValue(serializeItem<uint32_t>& v) { return _msg->bind<uint64_t, uint32_t>(&PBDecoder::convertValue, v); }

bool PBDecoder::decodeValue(serializeItem<int64_t>& v) { return _msg->bind<uint64_t, int64_t>(&PBDecoder::convertValue, v); }

bool PBDecoder::decodeValue(serializeItem<uint64_t>& v) { return _msg->bind<uint64_t, uint64_t>(&PBDecoder::convertValue, v); }

bool PBDecoder::decodeValue(serializeItem<float>& v) { return _msg->bind<uint32_t, float>(&PBDecoder::convertValue, v); }

bool PBDecoder::decodeValue(serializeItem<double>& v) { return _msg->bind<uint64_t, double>(&PBDecoder::convertValue, v); }

bool PBDecoder::decodeValue(serializeItem<std::string>& v) { return _msg->bind<proto::bin_type, std::string>(&PBDecoder::convertValue, v); }
...
bool PBDecoder::decodeRepaeted(serializeItem<std::vector<float> >& v) { return _msg->bind<uint64_t, std::vector<float> >(&PBDecoder::convertArray, v); }

bool PBDecoder::decodeRepaeted(serializeItem<std::vector<double> >& v) { return _msg->bind<uint64_t, std::vector<double> >(&PBDecoder::convertArray, v); }

bool PBDecoder::decodeRepaeted(serializeItem<std::vector<std::string> >& v) { return _msg->bind<proto::bin_type, std::vector<std::string> >(&PBDecoder::convertArray, v); }
