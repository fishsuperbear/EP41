...
JSONDecoder::JSONDecoder(const char* str, bool caseInsensitive) : _convertByType(true), _caseInsensitive(caseInsensitive), _cur(custom::GenericReader(_vec).Parse(str)) { assert(_cur); }
...
void JSONDecoder::decodeValue(const char* name, uint8_t& value, bool* pHas) {
    const custom::GenericValue* item = custom::GenericReader::GetObjectItem(_cur, name, _caseInsensitive);
    if (item && checkItemType(*item, custom::GenericValue::VALUE_NUMBER)) {
        value = (uint8_t)custom::GenericReader::convertUint(item->value, item->valueSize);
        if (pHas) *pHas = true;
    }
}

void JSONDecoder::decodeValue(const char* name, int8_t& value, bool* pHas) {
    const custom::GenericValue* item = custom::GenericReader::GetObjectItem(_cur, name, _caseInsensitive);
    if (item && checkItemType(*item, custom::GenericValue::VALUE_NUMBER)) {
        value = (int8_t)custom::GenericReader::convertInt(item->value, item->valueSize);
        if (pHas) *pHas = true;
    }
}
void JSONDecoder::decodeValue(const char* name, uint16_t& value, bool* pHas) {
    const custom::GenericValue* item = custom::GenericReader::GetObjectItem(_cur, name, _caseInsensitive);
    if (item && checkItemType(*item, custom::GenericValue::VALUE_NUMBER)) {
        value = (uint16_t)custom::GenericReader::convertUint(item->value, item->valueSize);
        if (pHas) *pHas = true;
    }
}

void JSONDecoder::decodeValue(const char* name, int16_t& value, bool* pHas) {
    const custom::GenericValue* item = custom::GenericReader::GetObjectItem(_cur, name, _caseInsensitive);
    if (item && checkItemType(*item, custom::GenericValue::VALUE_NUMBER)) {
        value = (int16_t)custom::GenericReader::convertInt(item->value, item->valueSize);
        if (pHas) *pHas = true;
    }
}
...
void JSONDecoder::decodeValue(const char* name, float& value, bool* pHas) {
    const custom::GenericValue* item = custom::GenericReader::GetObjectItem(_cur, name, _caseInsensitive);
    if (item && checkItemType(*item, custom::GenericValue::VALUE_NUMBER)) {
        value = static_cast<float>(custom::GenericReader::convertDouble(item->value, item->valueSize));
        if (pHas) *pHas = true;
    }
}
...
void JSONDecoder::decodeValue(const char* name, std::string& value, bool* pHas) {
    ...
    if (item && checkItemType(*item, custom::GenericValue::VALUE_STRING) && item->value && item->valueSize) {
        value.clear();
        parse_string(value, item->value, item->valueSize);
        if (pHas) *pHas = true;
    }
}
