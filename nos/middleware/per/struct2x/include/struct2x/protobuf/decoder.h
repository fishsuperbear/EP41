 ...
 *
 */
#ifndef MIDDLEWARE_PER_STRUCT2X_INCLUDE_STRUCT2X_PROTOBUF_DECODER_H_
#define MIDDLEWARE_PER_STRUCT2X_INCLUDE_STRUCT2X_PROTOBUF_DECODER_H_
...
struct convertVarint {
    static inline T value(const P& cValue) { return T(cValue); }
};
template <>
struct convertVarint<float, uint32_t> {
    static inline float value(const uint32_t& cValue) {
        union {
            uint32_t i;
            float f;
        };
...
template <>
struct convertVarint<double, uint64_t> {
    static inline double value(const uint64_t& cValue) {
        union {
            uint64_t i;
            double db;
        };
...
class EXPORTAPI Message {
    ...
    class converter {
     ...
     public:
        ...
        bool operator()(uint8_t* pStruct, const void* cValue) const { return (*_func)(pStruct + _offset, cValue, _type, reinterpret_cast<bool*>(_has ? pStruct + _has : NULL)); }
 ...
 public:
    ...
    bool bind(bool (*f)(T&, const P&, const uint32_t, bool*), struct2x::serializeItem<T>& value) {
        offset_type offset = reinterpret_cast<uint8_t*>(&value.value) - _struct;
        offset_type has = value.bHas ? (reinterpret_cast<uint8_t*>(value.bHas)) - _struct : 0;
...
class EXPORTAPI PBDecoder {
 ...
 public:
    ...
    PBDecoder& operator&(serializeItem<T> value) {
        if (_bParseResult) _bParseResult = decodeValue(*(serializeItem<typename internal::TypeTraits<T>::Type>*)(&value));
        return *this;
    }
    ...
    PBDecoder& operator&(serializeItem<std::vector<T> > value) {
        ...
        if (_bParseResult) _bParseResult = decodeRepaeted(*(serializeItem<std::vector<typename internal::TypeTraits<T>::Type> >*)(&value));
        return *this;
    }
    ...
    PBDecoder& operator&(serializeItem<std::map<K, V> > value) {
        if (!value.value.empty()) value.value.clear();
        if (_bParseResult) _bParseResult = _msg->bind<proto::bin_type, K, V>(&PBDecoder::convertMap, value);
        return *this;
    }
    template <typename V>
 ...
 private:
    ...
    static bool convertArray(std::vector<T>& value, const proto::bin_type& cValue, const uint32_t type, bool* pHas) {
        ...
        while (remaining) {
            uint64_t varint = 0;
            if (!proto::Message::ReadVarInt(current, remaining, varint)) return false;
    ...
    static bool convertCustom(T& value, const proto::bin_type& cValue, const uint32_t type, bool* pHas) {
        ...
        if (!decoder.operator>>(value)) return false;
        if (pHas) *pHas = true;
        return true;
    }
    ...
    static bool convertCustomArray(std::vector<T>& value, const proto::bin_type& cValue, const uint32_t type, bool* pHas) {
        struct2x::PBDecoder decoder(cValue.first, cValue.second);
        T temp = T();
        if (!decoder.operator>>(temp)) return false;
    ...
    static bool convertMap(std::map<K, V>& value, const proto::bin_type& cValue, const uint32_t type, bool* pHas) {
        ...
        if (msg.empty()) {
            ...
            msg.offset(1, (proto::offset_type)(reinterpret_cast<uint8_t*>(&key)));
            msg.offset(2, (proto::offset_type)(reinterpret_cast<uint8_t*>(&v)));
        }
        if (!msg.ParseFromBytes(decoder._sz, decoder._size, NULL)) return false;
...
#endif  // MIDDLEWARE_PER_STRUCT2X_INCLUDE_STRUCT2X_PROTOBUF_DECODER_H_
