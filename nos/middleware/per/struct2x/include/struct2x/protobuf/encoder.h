 ...
 *
 */
#ifndef MIDDLEWARE_PER_STRUCT2X_INCLUDE_STRUCT2X_PROTOBUF_ENCODER_H_
#define MIDDLEWARE_PER_STRUCT2X_INCLUDE_STRUCT2X_PROTOBUF_ENCODER_H_
...
#define BEGINCALCULATEFIELD(buf) std::pair<bool, size_t> customField(buf.getCustomField());

#define ENDCALCULATEFIELD(nByteSize, buf)    \
    nByteSize = buf.getCustomField().second; \
    buf.setCustomField(customField);
...
class convertMgr {
    ...
    class converter {
     ...
     public:
        converter(convert_t func, uint64_t tag, uint32_t type, offset_type offset, offset_type has) : _func(func), _offset(offset), _has(has), _info(encodeVarint(tag, type)) {}
        void operator()(const void* pStruct, BufferWrapper& buf) const { (*_func)((const uint8_t*)pStruct + _offset, (const bool*)(_has ? (const uint8_t*)pStruct + _has : NULL), _info, buf); }
    };
 ...
 private:
    ...
    static void encodeValue(const std::map<K, V>& value, const bool* pHas, const proto::enclosure_t& info, proto::BufferWrapper& buf) {
        for (typename std::map<K, V>::const_iterator it = value.begin(); it != value.end(); ++it) {
            ...
            static encodeFunction convsetSet[] = {NULL, (encodeFunction)&PBEncoder::encodeValueSvarint32, (encodeFunction)&PBEncoder::encodeValueFixed32,
                                                  (encodeFunction)&PBEncoder::encodeValueFixed64};
...
#endif  // MIDDLEWARE_PER_STRUCT2X_INCLUDE_STRUCT2X_PROTOBUF_ENCODER_H_
