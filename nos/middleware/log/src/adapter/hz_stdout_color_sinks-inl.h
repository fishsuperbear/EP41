...
#    include <spdlog/sinks/hz_stdout_color_sinks.h>
#endif
...
{
    return Factory::template create<sinks::hz_stdout_color_sink_mt>(logger_name, mode);
}

template<typename Factory>
SPDLOG_INLINE std::shared_ptr<logger> stdout_color_st(const std::string &logger_name, color_mode mode)
{
    return Factory::template create<sinks::hz_stdout_color_sink_st>(logger_name, mode);
}
