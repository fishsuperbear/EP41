...
Status McapReader::open(IReadable& reader) {
  //printf("McapReader::open(IReadable& reader)\n");
  reset_();
...
Status McapReader::open(std::string_view filename) {
  // printf("McapReader::open(std::string_view filename)\n");
  if (file_) {
    std::fclose(file_);
    file_ = nullptr;
  }
...
Status McapReader::open(std::ifstream& stream) {
  // printf("McapReader::open(std::ifstream& stream)\n");
  fileStreamInput_ = std::make_unique<FileStreamReader>(stream);
  // if (fileStreamInput_ == nullptr) {
  //   printf("McapReader::open(std::ifstream& stream) : fileStreamInput_ is nullptr\n");
  // } else {
  //   printf("McapReader::open(std::ifstream& stream) : fileStreamInput_ is init success\n");
  // }
...
Status McapReader::readSummarySection_(IReadable& reader) {
  ...
  typedReader.onAttachmentIndex = [&](const AttachmentIndex& attachmentIndex, ByteOffset) {
    // printf("attachmentIndex.name : %s\n", attachmentIndex.name.c_str());
...
Status McapReader::readSummaryFromScan_(IReadable& reader) {
  ...
  typedReader.onAttachment = [&](const Attachment& attachment, ByteOffset fileOffset) {
    ...
    // printf("attachmentIndex.name : %s, fileOffset : %lu\n", attachmentIndex.name.c_str(), fileOffset);
...
AttachmentPtr McapReader::readAttachment(const std::string &name) {
  // printf("------------------------McapReader::readAttachment :-------------------------------\n");
  for (const auto& index : attachmentIndexes_) {
    if (index.first == name) {
      AttachmentPtr attachment = std::make_shared<Attachment>();
      attachment->logTime = index.second.logTime;
      attachment->createTime = index.second.createTime;
      attachment->name = index.second.name;
      attachment->mediaType = index.second.mediaType;
      attachment->dataSize = index.second.dataSize;
      Record record;
      Status status;
      // ReadRecord(IReadable& reader, uint64_t offset, Record* record)
      if (fileStreamInput_ != nullptr) {
        status = McapReader::ReadRecord(*fileStreamInput_, index.second.offset, &record);
      } else if (fileInput_ != nullptr) {
        status = McapReader::ReadRecord(*fileInput_, index.second.offset, &record);
      } else if (input_ != nullptr) {
        status = McapReader::ReadRecord(*input_, index.second.offset, &record);
      }
      if (status.ok()) {
        status = ParseAttachment(record, attachment.get());
        if (status.ok()) {
          // printf("attachmentindex_length : %lu\n", index.second.length);
          // printf("attachment_logTime : %lu,attachmentindex_logTime : %lu\n", attachment->logTime, index.second.logTime);
          // printf("attachment_datasize : %lu,attachmentindex_datasize : %lu\n", attachment->dataSize, index.second.dataSize);
          // printf("attachment_name : %s\n", attachment->name.c_str());
          // printf("attachment_data : %s\n", reinterpret_cast<const char*>(attachment->data));
          return attachment;
        }
      }
    }
...
const std::multimap<std::string, AttachmentIndex> McapReader::attachments() const {
...
Status McapReader::ReadRecord(IReadable& reader, uint64_t offset, Record* record) {
  ...
  if (bytesRead != 9) {
    // printf("error : ReadFailed , bytesRead != 9");
    return StatusCode::ReadFailed;
  }
  ...
  // printf("McapReader::ReadRecord StatusCode::Success, OpCode : 0x%02x\n", (unsigned int)record->opcode);
  return StatusCode::Success;
}
