...
{
  COMMON_LOG_ERROR << LOG_NAME << status.message;
}
...
#endif
  void RegisterSplitFileCallback(
    std::function<void(std::string, uint64_t, bool)> callback_func) override;
...
private:
  ...
  // std::shared_ptr<rosbag2_storage::Attachment> attachmentPtr;
};
                            ...
                            const std::string & storage_config_uri)
{
  switch (io_flag) {
    ...
    case rosbag2_storage::storage_interfaces::IOFlag::APPEND: {
      ...
      relative_path_ = uri + FILE_EXTENSION + ".active";
...
rosbag2_storage::BagMetadata MCAPStorage::get_metadata()
{
  ...
  for (const auto & attachment : mcap_reader_->attachments()) {
    rosbag2_storage::FileInformation file;
    file.path = attachment.first;
    file.starting_time = time_point(std::chrono::nanoseconds(attachment.second.logTime));
    file.duration = std::chrono::nanoseconds(0);
    file.message_count = 1;
    file.file_type = attachment.second.mediaType;
    metadata_.files.emplace_back(file);
  }

  //获取app version
  metadata_.app_version = get_app_version();
  return metadata_;
}

std::string MCAPStorage::get_app_version()
{
  std::vector<std::string> attach_list;
  get_all_attachments_filepath(attach_list);
  if (std::find(attach_list.begin(), attach_list.end(), "/app/version.json") != attach_list.end()) {
    rosbag2_storage::AttachmentSharedPtr atta_ptr = read_Attachment("/app/version.json");
    return atta_ptr->data;
  }
  return "";
}
...
std::shared_ptr<rosbag2_storage::Attachment> MCAPStorage::read_Attachment(std::string name)
{
  auto attachment = mcap_reader_->readAttachment(name);
  if (attachment == nullptr) return nullptr;
  auto attachmentPtr = std::make_shared<rosbag2_storage::Attachment>();
  attachmentPtr->logTime = attachment->logTime;
  attachmentPtr->createTime = attachment->createTime;
  attachmentPtr->name = attachment->name;
  attachmentPtr->mediaType = attachment->mediaType;
  attachmentPtr->dataSize = attachment->dataSize;
  attachmentPtr->data.assign(reinterpret_cast<const char *>(attachment->data),
                             attachment->dataSize);
  return attachmentPtr;
}

void MCAPStorage::get_all_attachments_filepath(std::vector<std::string> & attach_list)
{
  // printf("MCAPStorage::get_all_attachments_filepath : \n");
  auto ret_list = mcap_reader_->attachments();
  for (const auto & attachment : ret_list) {
    // printf("[MCAPStorage] : %s\n", attachment.first.c_str());
    attach_list.emplace_back(attachment.first);
  }
}
...
void MCAPStorage::write(std::shared_ptr<const rosbag2_storage::SerializedBagMessage> msg)
{
  ...
  mcap_msg.sequence = 0;
  if (msg->time_stamp < 0) {
    COMMON_LOG_WARN << LOG_NAME << "Invalid message timestamp " << msg->time_stamp;
  }
...
void MCAPStorage::add_attach(std::shared_ptr<const rosbag2_storage::Attachment> attachment)
{
  mcap::Attachment mcap_attachment;
  mcap_attachment.logTime = attachment->logTime;
  mcap_attachment.createTime = attachment->createTime;
  mcap_attachment.name = attachment->name;
  mcap_attachment.mediaType = attachment->mediaType;
  mcap_attachment.dataSize = attachment->dataSize;
  mcap_attachment.data = reinterpret_cast<const std::byte *>(attachment->data.data());
  mcap_writer_->write(mcap_attachment);
}
...
void MCAPStorage::create_topic(const rosbag2_storage::TopicMetadata & topic)
{
  ...
  if (topic_it == topics_.end()) {
    ...
    COMMON_LOG_WARN << LOG_NAME << "Topic with name: " << topic.name << " already exist!";
    return;
  }
  ...
  if (schema_it == schema_ids_.end()) {
    ...
    try {
      ...
      // COMMON_LOG_WARN << LOG_NAME << "definition file(s) missing for " << datatype << ": missing "
      //                 << err.what();
      schema.encoding = "";
    }
...
void MCAPStorage::RegisterSplitFileCallback(
  std::function<void(std::string, uint64_t, bool)> callback_func)
{
  if (callback_func) {
  }

  COMMON_LOG_WARN << "RegisterSplitFileCallback should not be called";
}
