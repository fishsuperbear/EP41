...
class ROSBAG2_STORAGE_PUBLIC StorageFactoryInterface {
   public:
    ...
    virtual std::shared_ptr<storage_interfaces::ReadWriteInterface> open_read_write(const StorageOptions& storage_options,
                                                                                    std::function<void(std::string, uint64_t, bool)> callback_func = nullptr) = 0;
};
