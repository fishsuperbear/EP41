...
T decode_for_version(const Node& node, int version) {
    static_assert(std::is_default_constructible<T>::value, "Type passed to decode_for_version that has is not default constructible.");
    if (!node.IsDefined()) {
...
struct convert<rosbag2_storage::TopicInformation> {
    ...
    static bool decode(const Node& node, rosbag2_storage::TopicInformation& metadata, int version) {
        metadata.topic_metadata = decode_for_version<rosbag2_storage::TopicMetadata>(node["topic_metadata"], version);
...
struct convert<std::vector<rosbag2_storage::TopicInformation>> {
    ...
    static bool decode(const Node& node, std::vector<rosbag2_storage::TopicInformation>& rhs, int version) {
        if (!node.IsSequence()) {
            return false;
        }
...
struct convert<std::chrono::time_point<std::chrono::high_resolution_clock>> {
    ...
    static bool decode(const Node& node, std::chrono::time_point<std::chrono::high_resolution_clock>& start_time) {
        start_time = std::chrono::time_point<std::chrono::high_resolution_clock>(std::chrono::nanoseconds(node["nanoseconds_since_epoch"].as<uint64_t>()));
        return true;
    }
};
...
struct convert<rosbag2_storage::BagMetadata> {
    ...
    static bool decode(const Node& node, rosbag2_storage::BagMetadata& metadata) {
        ...
        metadata.starting_time = node["starting_time"].as<std::chrono::time_point<std::chrono::high_resolution_clock>>();
        metadata.message_count = node["message_count"].as<uint64_t>();
        metadata.topics_with_message_count = decode_for_version<std::vector<rosbag2_storage::TopicInformation>>(node["topics_with_message_count"], metadata.version);
...
BagMetadata MetadataIo::read_metadata(const std::string& uri) {
    try {
        ...
        if (RCUTILS_RET_OK != rcutils_calculate_directory_size(uri.c_str(), &metadata.bag_size, allocator)) {
            throw std::runtime_error(std::string("Exception on calculating the size of directory :") + uri);
        }
        return metadata;
