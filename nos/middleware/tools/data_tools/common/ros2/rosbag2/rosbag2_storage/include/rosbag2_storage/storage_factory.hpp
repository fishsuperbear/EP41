...
class ROSBAG2_STORAGE_PUBLIC StorageFactory : public StorageFactoryInterface {
   public:
    ...
    std::shared_ptr<storage_interfaces::ReadOnlyInterface> open_read_only(const StorageOptions& storage_options) override;

    std::shared_ptr<storage_interfaces::ReadWriteInterface> open_read_write(const StorageOptions& storage_options, std::function<void(std::string, uint64_t, bool)> callback_func = nullptr) override;
