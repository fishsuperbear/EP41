...
struct StorageOptions {
   public:
    ...
    uint64_t max_files = 864000;

    // The maximum size in byte a bagfile can be, in bytes, before it is split.
    // A value of 0 indicates that bagfile splitting will not be used.
    uint64_t max_bagfile_size = 2 * 1024 * 1024 * 1024ULL;

    // The maximum duration a bagfile can be, in seconds, before it is split.
    // A value of 0 indicates that bagfile splitting will not be used.
    uint64_t max_bagfile_duration = 1 * 60 * 60ULL;
