...
class RecordWriter : public RecordBase {
   ...
   */
    bool Open(const std::string& file, const uint64_t& max_files);
   ...
   */
    ...
    /**
   * @brief registe spit file callback(hz code)
   */
    void RegisterSplitFileCallback(std::function<void(std::string, uint64_t, bool)> callback_func);
   ...
   private:
    ...
    void NotifyNewFile(bool is_open);                          //hz添加代码
    void pre_bag_remove_active_suffix(std::string& per_path);  //hz添加代码
    ...
    uint64_t max_files_ = 0;  //hozon code

    std::vector<std::function<void(std::string, uint64_t, bool)>> split_file_callback_;
};

template <>
inline bool RecordWriter::WriteMessage(const std::string& channel_name, const std::string& message, const uint64_t time_nanosec, const std::string& proto_desc) {
...
template <>
inline bool RecordWriter::WriteMessage(const std::string& channel_name, const std::shared_ptr<message::RawMessage>& message, const uint64_t time_nanosec, const std::string& proto_desc) {
    if (message == nullptr) {
...
template <typename MessageT>
bool RecordWriter::WriteMessage(const std::string& channel_name, const MessageT& message, const uint64_t time_nanosec, const std::string& proto_desc) {
    ...
    if (message_type.empty()) {
        ...
        if (MessageT::descriptor()->full_name() != message_type) {
            AERROR << "Message type is invalid, expect: " << message_type << ", actual: " << message::GetMessageName<MessageT>();
            return false;
        }
    }
    std::string content("");
