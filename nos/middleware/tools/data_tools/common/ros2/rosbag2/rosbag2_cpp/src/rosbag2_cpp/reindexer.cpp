  ...
  std::vector<rcpputils::fs::path> & output)
{
  ...
  do {
    ...
    ROSBAG2_CPP_LOG_DEBUG << "Found file: " << found_file.string();
  ...
  const rosbag2_storage::StorageOptions & storage_options)
{
  ...
  ROSBAG2_CPP_LOG_DEBUG << "Extracting metadata from database(s)";
  for (const auto & f_ : files) {
    ROSBAG2_CPP_LOG_DEBUG << "Extracting from file: " + f_.string();
    ...
    ROSBAG2_CPP_LOG_DEBUG << "New duration: " + std::to_string(metadata_.duration.count());
    ...
    for (const auto & topic : temp_metadata.topics_with_message_count) {
      ...
      if (found_topic == temp_topic_info.end()) {
        ...
        ROSBAG2_CPP_LOG_DEBUG << "Found topic!";
...
void Reindexer::reindex(const rosbag2_storage::StorageOptions & storage_options)
{
  ...
  ROSBAG2_CPP_LOG_INFO << "Beginning reindexing bag in directory: " << base_folder_.string();
  ...
  ROSBAG2_CPP_LOG_DEBUG << "Completed init_metadata";

  // Collect all metadata from database files
  aggregate_metadata(files, bag_reader, storage_options);
  ROSBAG2_CPP_LOG_DEBUG << "Completed aggregate_metadata";
