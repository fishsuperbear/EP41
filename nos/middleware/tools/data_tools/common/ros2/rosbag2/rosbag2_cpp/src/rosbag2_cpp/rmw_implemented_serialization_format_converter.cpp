...
T get_function_from(const char* function_name, std::shared_ptr<rcpputils::SharedLibrary> library) {
    if (!library->has_symbol(function_name)) {
        std::stringstream ss;
        ss << "Converter could not find expected symbol '" << function_name << "' in rmw implementation library " << library->get_library_path();
...
class RMWImplementedConverterImpl {
   public:
    explicit RMWImplementedConverterImpl(const std::string& format) {
        ...
        ROSBAG2_CPP_LOG_ERROR << "call RMWImplementedConverterImpl::RMWImplementedConverterImpl()! format=" << format;
        return;
    }
...
RMWImplementedConverter::RMWImplementedConverter(const std::string& format) : impl_(std::make_unique<RMWImplementedConverterImpl>(format)) {}
