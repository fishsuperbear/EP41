...
namespace readers {
namespace details {
std::vector<std::string> resolve_relative_paths(const std::string& base_folder, std::vector<std::string> relative_files, const int version = 4) {
    ...
    rcpputils::require_true(base_path.is_directory(), "base folder has to be a directory: " + base_folder);
...
SequentialReader::SequentialReader(std::unique_ptr<rosbag2_storage::StorageFactoryInterface> storage_factory, std::shared_ptr<SerializationFormatConverterFactoryInterface> converter_factory,
                                   std::unique_ptr<rosbag2_storage::MetadataIo> metadata_io)
    : storage_factory_(std::move(storage_factory)), converter_(nullptr), metadata_io_(std::move(metadata_io)), converter_factory_(std::move(converter_factory)) {}
...
bool SequentialReader::open(const rosbag2_storage::StorageOptions& storage_options, const ConverterOptions& converter_options) {

    if ("mcap" != storage_options.storage_id) {
        COMMON_LOG_ERROR << "input file format:" << storage_options.storage_id << " is unsupported.";
        return false;
    }
    ...
    if (metadata_io_->metadata_file_exists(storage_options.uri)) {
        ...
        file_paths_ = details::resolve_relative_paths(storage_options.uri, metadata_.relative_file_paths, metadata_.version);
        current_file_iterator_ = file_paths_.begin();
        load_current_file();
    } else {
        rcpputils::fs::path t_path = rcpputils::fs::path(base_folder_);
        if (!rcpputils::fs::exists(rcpputils::fs::path(t_path))) {
            COMMON_LOG_ERROR << base_folder_ << " does not exist.";
            return false;
        }
        if (t_path.file_size() <= 0) {
            COMMON_LOG_ERROR << "can't open " << base_folder_ << " . size is 0.";
            return false;
        }
        if (!t_path.is_readable()) {
            COMMON_LOG_ERROR << "can't open " << base_folder_ << " . unreadable.";
            return false;
        }

        storage_ = storage_factory_->open_read_only(storage_options_);
        if (!storage_) {
            COMMON_LOG_ERROR << "open storage failed.";
            return false;
        }
    ...
    check_converter_serialization_format(converter_options.output_serialization_format, topics[0].topic_metadata.serialization_format);
    return true;
}
...
bool SequentialReader::has_next() {
    ...
    COMMON_LOG_ERROR << "Bag is not open. Call open() before reading.";
    return false;
}
...
std::shared_ptr<rosbag2_storage::SerializedBagMessage> SequentialReader::read_next() {
    if (storage_) {
        ...
        COMMON_LOG_ERROR << "Bag is at end. No next message.";
        return nullptr;
    }
    COMMON_LOG_ERROR << "Bag is not open. Call open() before reading.";
    return nullptr;
...
void SequentialReader::check_topics_serialization_formats(const std::vector<rosbag2_storage::TopicInformation>& topics) {
...
void SequentialReader::check_converter_serialization_format(const std::string& converter_serialization_format, const std::string& storage_serialization_format) {
    if (converter_serialization_format.empty()) {
        return;
    }

    if (converter_serialization_format != storage_serialization_format) {
        converter_ = std::make_unique<Converter>(storage_serialization_format, converter_serialization_format, converter_factory_);
...
void SequentialReader::add_event_callbacks(const bag_events::ReaderEventCallbacks& callbacks) {
    if (callbacks.read_split_callback) {
        callback_manager_.add_event_callback(callbacks.read_split_callback, bag_events::BagEvent::READ_SPLIT);
    }
}
...
std::shared_ptr<rosbag2_storage::Attachment> SequentialReader::read_Attachment(std::string name) {
    return storage_->read_Attachment(name);
}

void SequentialReader::get_all_attachments_filepath(std::vector<std::string>& attach_list) {
    storage_->get_all_attachments_filepath(attach_list);
}
