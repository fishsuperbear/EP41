 ...
 */
class ROSBAG2_CPP_PUBLIC SequentialWriter : public rosbag2_cpp::writer_interfaces::BaseWriterInterface {
   public:
    explicit SequentialWriter(std::unique_ptr<rosbag2_storage::StorageFactoryInterface> storage_factory = std::make_unique<rosbag2_storage::StorageFactory>(),
                              std::shared_ptr<SerializationFormatConverterFactoryInterface> converter_factory = std::make_shared<SerializationFormatConverterFactory>(),
   ...
   **/
    void open(const rosbag2_storage::StorageOptions& storage_options, const ConverterOptions& converter_options) override;
   ...
   */
    ...
    /**
   * \brief handle record event.
   * \param
   */
    void handle_record_event(std::string absolut_filr_path, uint64_t time, bool is_open);

    /**
   * \brief write attachments.
   * \param
   */
    void add_attach(std::shared_ptr<rosbag2_storage::Attachment> attachment) override;
    /**
   * \brief save attachments to list to write.
   * \param
   */
    void add_attach_to_list(std::shared_ptr<rosbag2_storage::Attachment>& attachment) override;
    /**
   * \brief splite bag immedialy.
   * \param
   */
    void splite_bag_now();
   ...
   protected:
    ...
    std::vector<std::shared_ptr<rosbag2_storage::Attachment>> attachments_list_;
    ...
    void switch_to_next_storage(const std::string& time_suffix = "");

    // if exceed the max files, delete the oldest file.
    void deleteOldestFile();
    //去除文件的.active 后缀
    void pre_bag_remove_active_suffix(std::string& per_path);

    std::string format_storage_uri(const std::string& base_folder, const std::string& storage_name, uint64_t storage_count, const std::string& time_suffix = "");
    ...
    virtual void split_bagfile(const std::string& time_suffix = "");
   ...
   private:
    ...
    void write_messages(const std::vector<std::shared_ptr<const rosbag2_storage::SerializedBagMessage>>& messages);
