...
Reader::Reader(std::unique_ptr<reader_interfaces::BaseReaderInterface> reader_impl) : reader_impl_(std::move(reader_impl)) {}

Reader::~Reader() {
    reader_impl_->close();
}

bool Reader::open(const std::string& uri) {
...
bool Reader::open(const rosbag2_storage::StorageOptions& storage_options, const ConverterOptions& converter_options) {
    return reader_impl_->open(storage_options, converter_options);
}
...
std::shared_ptr<rosbag2_storage::Attachment> Reader::read_Attachment(const std::string name) {
    return reader_impl_->read_Attachment(name);
}

void Reader::get_all_attachments_filepath(std::vector<std::string>& attach_list) {
    reader_impl_->get_all_attachments_filepath(attach_list);
}
