...
#include <string>
#include <vector>
#include "rcpputils/function_traits.hpp"
...
class BagEventCallback : public BagEventCallbackBase {
   public:
    BagEventCallback(const EventCallbackT& callback, BagEvent event) : callback_(callback), event_(event) {}

    virtual ~BagEventCallback() {}

    void execute(InfoPtr& info) override { callback_(*std::static_pointer_cast<EventCallbackInfoT>(info)); }

    bool is_type(BagEvent event) const override { return event == event_; }

   private:
    using EventCallbackInfoT = typename std::remove_reference<typename rcpputils::function_traits::function_traits<EventCallbackT>::template argument_type<0>>::type;
