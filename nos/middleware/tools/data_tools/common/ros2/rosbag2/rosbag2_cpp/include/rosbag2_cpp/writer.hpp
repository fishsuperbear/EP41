...
class ROSBAG2_CPP_PUBLIC Writer final {
   ...
   */
    void write(const eprosima::fastrtps::rtps::SerializedPayload_t& message, const std::string& topic_name, const std::string& type_name, const int64_t& time);


    void add_attach(std::shared_ptr<rosbag2_storage::Attachment> attachment);
    // /**
    //  * Write a serialized message to a bagfile.
    //  * The topic will be created if it has not been created already.
    //  *
    ...
    writer_interfaces::BaseWriterInterface& get_implementation_handle() const { return *writer_impl_; }
