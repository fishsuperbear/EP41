...
hozon::soc::CompressedImage CameraEncodedFrameToImageOut(CameraEncodedMbufStructHelp mdc_data) {
    ...
    proto_data.mutable_header()->set_publish_stamp(static_cast<double>(mdc_data.RealStruct.CameraHeader.Stamp.Sec) + static_cast<double>(mdc_data.RealStruct.CameraHeader.Stamp.Nsec) / 1e9);
