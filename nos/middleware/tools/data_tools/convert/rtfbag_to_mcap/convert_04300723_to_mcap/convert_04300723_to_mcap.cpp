...
class Convert_04300723_ToMcap : public ConvertBase {
   private:
   ...
   public:
    ...
    Convert_04300723_ToMcap() : _callbackFunction(nullptr){};
    ~Convert_04300723_ToMcap(){};
};

void Convert_04300723_ToMcap::Convert(std::string input_file, const std::vector<std::string>& exclude_topics, const std::vector<std::string>& topics) {
    // 初始化rtfReader
...
std::vector<std::uint8_t> Convert_04300723_ToMcap::ConvertMessage(RtfBagReader::EventMsg event) {
    std::string eventName = event.GetEventName();
...
void Convert_04300723_ToMcap::RegistMessageCallback(std::function<void(std::string, std::string, int64_t time, std::vector<std::uint8_t>)> callbackFunction) {
    _callbackFunction = callbackFunction;
}

std::pair<std::string, std::string> Convert_04300723_ToMcap::ReadFileJson(std::string eventName) {
...
extern "C" ConvertBase* CreatConverter() {
    ConvertBase* temp = new Convert_04300723_ToMcap();
    return temp;
}

extern "C" void DestroyConverter(ConvertBase* ptr) {
    if (nullptr != ptr) {
        Convert_04300723_ToMcap* temp = static_cast<Convert_04300723_ToMcap*>(ptr);
        delete temp;
        ptr = nullptr;
    }
    return;
