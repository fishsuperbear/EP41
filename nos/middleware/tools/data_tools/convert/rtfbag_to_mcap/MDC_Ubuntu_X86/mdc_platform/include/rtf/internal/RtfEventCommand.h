...
#ifndef RTF_EVENT_COMMAND_H
#define RTF_EVENT_COMMAND_H
...
namespace rtfevent {
// 错误码定义
constexpr int32_t RTF_EVENT_RET_OK      = 0;
constexpr int32_t RTF_EVENT_RET_FAILED  = -1;
constexpr int32_t RTF_EVENT_RET_HELP    = -2;

// 命令等级
constexpr unsigned long RTF_EVENT_FIRST_CMD   = 1UL; // 一级命令字，如：rtfevent
constexpr unsigned long RTF_EVENT_SECOND_CMD  = 2UL; // 二级命令字，如：rtfevent list中的list
constexpr unsigned long RTF_EVENT_THIRD_CMD   = 3UL; // 三级命令字，如：rtfevent list -h中的-h

// 命令名存储下标
constexpr unsigned int RTF_EVENT_FIRST_CMD_IDX   = 0U; // 一级命令字在vector中存储的下标，如：rtfevent
constexpr unsigned int RTF_EVENT_SECOND_CMD_IDX  = 1U; // 二级命令字在vector中存储的下标，如：rtfevent list中的list
constexpr unsigned int RTF_EVENT_THIRD_CMD_IDX   = 2U; // 二级命令字在vector中存储的下标，如：rtfevent list -h中的-h

// 命令行列表
enum RtfEventCmds : int32_t {
...
public:
    RtfEventCommand() noexcept;
    ~RtfEventCommand() override = default;
...
protected:
    // 打印命令帮助信息
    virtual void PrintHelpInfo() const noexcept;
