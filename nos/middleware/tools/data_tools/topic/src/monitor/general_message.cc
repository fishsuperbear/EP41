 ...
 */
std::vector<int> SortProtobufMapByKeys(const google::protobuf::Message& message, const google::protobuf::FieldDescriptor* field, const google::protobuf::Reflection& reflection, const int size) {
...
GeneralMessage::GeneralMessage(GeneralMessageBase* parent, const google::protobuf::Message* msg, const google::protobuf::Reflection* reflection, const google::protobuf::FieldDescriptor* field)
    : GeneralMessageBase(parent), item_index_(0), is_folded_(true), field_(field), message_ptr_(msg), reflection_ptr_(reflection) {}
...
int GeneralMessage::Render(const Screen* s, int key) {
    ...
    int line_no = 0;
    {
        ...
        s->AddStr(channel_msg_ptr->topic_name().c_str());

        s->AddStr(0, line_no++, "ProtoType: ");
        s->AddStr(channel_msg_ptr->proto_type().c_str());
        ...
        auto channel_msg = channel_msg_ptr->CopyMsg();
        if (!channel_msg_ptr->raw_msg_class_->ParseFromArray(channel_msg.data(), channel_msg.size())) {
        ...
        if (message_ptr_ && reflection_ptr_) {
            ...
            if (is_folded_) {
                GeneralMessageBase::PrintField(this, *message_ptr_, &jump_lines, s, &line_no, 0, reflection_ptr_, field_, indices[item_index_]);
            } else {
                for (const int index : indices) {
                    GeneralMessageBase::PrintField(this, *message_ptr_, &jump_lines, s, &line_no, 0, reflection_ptr_, field_, index);
                }
            }
        }
    }
