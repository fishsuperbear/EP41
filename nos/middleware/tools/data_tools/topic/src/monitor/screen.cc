...
const char Screen::InteractiveCmdStr[] =
    ...
    "   f | F -- show frame ratio for all topic messages\n"
    "   t | T -- show topic message type\n"
    "\n"
    "   Space -- Enable|Disable topic Message\n"
    "\n"
...
Screen::Screen() : current_color_pair_(INVALID), canRun_(false), current_state_(State::RenderMessage), highlight_direction_(0), current_render_obj_(nullptr) {}
...
void Screen::Run() {
    ...
    do {
        ...
        ch = SwitchState(ch);
        // std::cout << "current_state_=" << static_cast<int>(current_state_) << std::endl;  为 0
        (this->*showFuncs[static_cast<int>(current_state_)])(ch);

        double fr = current_render_obj_->frame_ratio();
        // std::cout << "*********fr=" << std::fixed << std::setprecision(4) << fr << std::endl;  为 0
