...
{
public:
    CryptoToolLog() : logger_(nullptr) {};
    virtual ~CryptoToolLog() {};
    ...
    void InitLogging(std::string appId = "DEFAULT_APP",  // the log id of application
        ...
        CryptoLogLevelType appLogLevel = CryptoLogLevelType::CRYPTO_DEBUG, //the log level of application
    ...
    static CryptoToolLog& GetInstance()
    {
        static CryptoToolLog instance;
        return instance;
    }
...
private:
    CryptoToolLog(const CryptoToolLog&);
    CryptoToolLog& operator=(const CryptoToolLog&);

public:
    std::shared_ptr<hozon::netaos::log::Logger>  logger_;
private:
    int32_t level_ = static_cast<int32_t>(CryptoLogLevelType::CRYPTO_DEBUG);
};

#define CRYTOOL_HEAD "pid: " << (long int)syscall(__NR_getpid) <<  " tid: " << (long int)syscall(__NR_gettid) << " | "
#define CRYTOOL_CRITICAL (CryptoToolLog::GetInstance().CryptoGetLogger()->LogCritical() << CRYTOOL_HEAD)
#define CRYTOOL_ERROR (CryptoToolLog::GetInstance().CryptoGetLogger()->LogError() << CRYTOOL_HEAD)
#define CRYTOOL_WARN (CryptoToolLog::GetInstance().CryptoGetLogger()->LogWarn() << CRYTOOL_HEAD)
#define CRYTOOL_INFO (CryptoToolLog::GetInstance().CryptoGetLogger()->LogInfo() << CRYTOOL_HEAD)
#define CRYTOOL_DEBUG (CryptoToolLog::GetInstance().CryptoGetLogger()->LogDebug() << CRYTOOL_HEAD)
#define CRYTOOL_TRACE (CryptoToolLog::GetInstance().CryptoGetLogger()->LogTrace() << CRYTOOL_HEAD)
