...
#include <algorithm>
#include <vector>
#include "adf-lite/include/freq_sta.h"
...
public:
    SizeLimitQueue(uint32_t capacity) :
            _capacity(capacity) {
...
public:
    CVSizeLimitQueue(uint32_t capacity = 5) :
            _queue(capacity) {
    ...
    std::shared_ptr<T> GetLatestOneBlocking(bool erase, const uint32_t timeout_ms = UINT32_MAX) {
        ...
        bool status = _cv.wait_for(recv_lk, std::chrono::milliseconds(timeout_ms), [this](){
            return _need_stop || (_new_data_arrived == true);
        });
        if (!status) {
            return nullptr;
        }
...
template<typename T>
class CVSizeLimitQueueWithFreq : public CVSizeLimitQueue<T> {
public:
    CVSizeLimitQueueWithFreq(uint32_t capacity = 5) :
        CVSizeLimitQueue<T>(capacity) {
        _freq_sta.Start();
    }

    void PushOneAndNotify(T data) override {
        std::unique_lock<std::mutex> recv_lk(CVSizeLimitQueue<T>::_mtx);
        if (!CVSizeLimitQueue<T>::_enable_write) {
            return;
        }

        _freq_sta.PushOnce();
        CVSizeLimitQueue<T>::_queue.PushOne(data);
        CVSizeLimitQueue<T>::_new_data_arrived = true;
        CVSizeLimitQueue<T>::_cv.notify_all();
    }

    FreqInfo ReadInfo() {
        return _freq_sta.ReadInfo();
    }
