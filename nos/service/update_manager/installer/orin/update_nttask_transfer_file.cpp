...
uint32_t UpdateNTTaskTransferFile::StartToMemoryErase()
{
    ...
    if (reqInfo.reqUpdateType == 1) {
        ...
        reqInfo.reqContent = { 0x31, 0x01, 0xFF, 0x00, 0x44, (uint8_t)(m_process.memoryAddr >> 24), (uint8_t)(m_process.memoryAddr >> 16),
            (uint8_t)(m_process.memoryAddr >> 8), (uint8_t)(m_process.memoryAddr), (uint8_t)(m_process.memorySize >> 24),
            (uint8_t)(m_process.memorySize >> 16), (uint8_t)(m_process.memorySize >> 8), (uint8_t)(m_process.memorySize) };
    } else if (reqInfo.reqUpdateType == 5) {
        reqInfo.reqContent = { 0x31, 0x01, 0xFF, 0x00, (uint8_t)(m_process.memoryAddr >> 24), (uint8_t)(m_process.memoryAddr >> 16),
            (uint8_t)(m_process.memoryAddr >> 8), (uint8_t)(m_process.memoryAddr), (uint8_t)(m_process.memorySize >> 24),
            (uint8_t)(m_process.memorySize >> 16), (uint8_t)(m_process.memorySize >> 8), (uint8_t)(m_process.memorySize) };
    }
...
void UpdateNTTaskTransferFile::OnMemoryEraseResult(STTask *task, uint32_t result)
{
    ...
    UPDATE_LOG_D("OnMemoryEraseResult ecu: %s, result: %s, progress: %d%%!~", m_sensorInfo.name.c_str(), GetTaskResultString(result).c_str(), m_progress);
...
{
    m_progress = m_progress + 1 <= 100 ? m_progress + 1 : 100;
    UPDATE_LOG_D("OnRequestBinDownloadResult ecu: %s, result: %s, progress: %d%%!~", m_sensorInfo.name.c_str(), GetTaskResultString(result).c_str(), m_progress);
    ...
    if (N_OK == result) {
        ...
        if (sftask->GetResInfo().resContent.size() == 4 || sftask->GetResInfo().resContent.size() == 6) {
            ...
            auto res = m_sensorInfo.name.find("SRR");
            if (m_reqBlockSize == 0x802 && res != std::string::npos) {
                m_reqBlockSize = 0x202;
            }
        }
        else {
...
void UpdateNTTaskTransferFile::OnTransFileResult(STTask *task, uint32_t result)
{
    ...
    UPDATE_LOG_D("OnTransFileResult ecu: %s, result: %s, progress: %d%%!~", m_sensorInfo.name.c_str(), GetTaskResultString(result).c_str(), m_progress);
    if (N_OK == result) {
        UpdateCTTaskCommand* sftask = static_cast<UpdateCTTaskCommand*>(task);
        if (nullptr == sftask) {
            result = N_ERROR;
...
{
    m_progress = m_progress + 1 <= 100 ? m_progress + 1 : 100;
    UPDATE_LOG_D("OnRequestTransExitResult ecu: %s, result: %s, progress: %d%%!~", m_sensorInfo.name.c_str(), GetTaskResultString(result).c_str(), m_progress);
...
{
    UPDATE_LOG_D("OnCheckDependencyResult ecu: %s, result: %s, m_progress: %d%%!~", m_sensorInfo.name.c_str(), GetTaskResultString(result).c_str(), m_progress);
