...
#define   SENSOR_UPDATE_RETRY_MAX_TIMES         (2)
...
UpdateNTTaskSensorInstaller::onCallbackAction(uint32_t result)
{
    ...
    if (eOK == result) {
        ...
        progress_ = 100;
        OTAStore::Instance()->WriteSensorVersionData(sensor_.sensorInfo.name, SensorEntityManager::Instance()->GetTargetVersion(sensor_.sensorInfo.name));
        OtaResultStore::Instance()->UpdateResult(sensor_.sensorInfo.name, "Succ");
        if (res != "DEFAULT") {
            // 启动timer 30min超时
        ...
        UPDATE_LOG_E("Task sensor: %s update completed failed, performance cost %0.3lf seconds!",
            sensor_.sensorInfo.name.c_str(), 0.001*(GetTickCount() - performance_));
        OtaResultStore::Instance()->UpdateResult(sensor_.sensorInfo.name, "Fail");
        if (res != "DEFAULT") {
            UpdateStateMachine::Instance()->SwitchState(State::OTA_UPDATE_FAILED, FailedCode::SENSOR_UPDATE_FAILED);
        }
    }
    OTARecoder::Instance().RecordFinish(sensor_.sensorInfo.name, result, progress_);
    CmdUpgradeManager::Instance()->SetEcuMode(0);
}
...
void UpdateNTTaskSensorInstaller::OnSendCommandResult(STTask* task, uint32_t result)
{
    ...
    UPDATE_LOG_D("OnSendCommandResult sensor: %s, result: %s, process: %d / total: %ld, progress: %d%% !~",
        sensor_.sensorInfo.name.c_str(), GetTaskResultString(result).c_str(), index_ + 1, sensor_.process.size(), progress_);
    ...
    if (N_OK == result) {
        ...
        if (sftask->GetResInfo().resContent.size() > 3 && sftask->GetResInfo().resContent[0] == 0x62) {
            ...
            if (resContent[1] == 0xF1 && resContent[2] == 0xC0) {
                // 需要重新加载一下配置
                auto res = ConfigManager::Instance().GetSensorManifestByName(sensor_.sensorInfo.name, sensor_);
                if (!res) {
                    UM_DEBUG << "GetSensorManifestByName error !";
                    result = N_ERROR;
                    onCallbackResult(result);
                    return;
                }
                std::string rightTargetVer = SensorEntityManager::Instance()->GetTargetVersion(sensor_.sensorInfo.name);

                // Read Software Version $62 F1 C0
                bool match = true;
                if (resContent.size() - 3 < rightTargetVer.size()) {
                    match = false;
                }
                for (uint8_t index = 0; index < rightTargetVer.size(); ++index) {
                    if (rightTargetVer[index] != resContent[index + 3]) {
                        match = false;
                        break;
                    }
                }
                std::string recvVersion = std::string(reinterpret_cast<const char*>(&resContent[3]), resContent.size() - 3);
                UPDATE_LOG_D("SoftwareVersion sameCheck: %d, target: %s, recv: %s", match, rightTargetVer.c_str(), recvVersion.c_str());
                OTARecoder::Instance().RecordUpdateVersion(sensor_.sensorInfo.name, recvVersion, rightTargetVer);
...
{
    progress_ = (sensor_.process)[index_].endProgress;
    UPDATE_LOG_D("OnSecurityAccessResult sensor: %s, result: %s, process: %d / total: %ld, progress: %d%% !~",
        sensor_.sensorInfo.name.c_str(), GetTaskResultString(result).c_str(), index_ + 1, sensor_.process.size(), progress_);
    OTARecoder::Instance().RecordStepFinish(sensor_.sensorInfo.name, (sensor_.process)[index_].updateStep, result, progress_);
    ++index_;
    if (N_OK == result) {
...
void UpdateNTTaskSensorInstaller::OnTransFileResult(STTask *task, uint32_t result)
{
    ...
    UPDATE_LOG_D("OnTransFileResult sensor: %s, result: %s, process: %d / total: %ld, progress: %d%% !~",
        sensor_.sensorInfo.name.c_str(), GetTaskResultString(result).c_str(), index_ + 1, sensor_.process.size(), progress_);
    OTARecoder::Instance().RecordStepFinish(sensor_.sensorInfo.name, (sensor_.process)[index_].updateStep, result, progress_);
    ++index_;
    if (N_OK == result) {
...
{
    UPDATE_LOG_D("OnFailRetryResult retry: %d, delay: %d ms, sensor: %s, result: %s, process: %d / total: %ld, progress: %d%% !~",
        retry_, OTA_TIMER_P3_CLIENT_PYH, sensor_.sensorInfo.name.c_str(), GetTaskResultString(result).c_str(), index_ + 1, sensor_.process.size(), progress_);
    if (N_OK == result) {
        index_ = 0;  // sensor failed retry no need check version and ecu info
        progress_ = 0;
        result = StartToUpdateProcess();
        if (eContinue != result) {
            onCallbackResult(result);
