...
template <class T>
static std::string ToStringDids(T t, std::ios_base & (*f)(std::ios_base&), int n = 0)
{
    std::ostringstream oss;
    if (n) {
        oss << std::setw(n) << std::setfill('0');
    ...
    if (1 == typesize) {
        uint8_t item = static_cast<uint8_t>(t);
        oss << f << std::uppercase << static_cast<uint16_t>(item);
    }
    else {
...
template <class T>
static std::string ToString(T t[], std::ios_base & (*f)(std::ios_base&), uint num = 0, uint pnum = PRINT_DATA_MAX_NUMBER, int n = 0)
{
    if ((nullptr == t) || (0 == num)) {
        return "";
    }
...
template <class T>
static std::string ToString(std::vector<T> t, std::ios_base & (*f)(std::ios_base&), uint pnum = PRINT_DATA_MAX_NUMBER, int n = 0)
{
    if (t.size() <= 0) {
        return "";
    }
...
#define UINT16_TO_STRING_DIDS(type) ToStringDids<uint16_t>(type, std::hex, 4)

#define UINT8_VEC_TO_STRING(vec) ToString<uint8_t>(vec, std::hex, 2)
#define UINT8_VEC_TO_STRING_P(vec, pnum) ToString<uint8_t>(vec, std::hex, pnum, 2)
#define UINT16_VEC_TO_STRING(vec) ToString<uint16_t>(vec, std::hex, 4)
#define UINT16_VEC_TO_STRING_P(vec, pnum) ToString<uint16_t>(vec, std::hex, pnum, 4)
#define UINT32_VEC_TO_STRING(vec) ToString<uint32_t>(vec, std::hex, 8)
#define UINT32_VEC_TO_STRING_P(vec, pnum) ToString<uint32_t>(vec, std::hex, pnum, 8)

#define CHAR_ARRAY_TO_STRING(array, num) ToString<char>(array, std::hex, num, 2)
#define CHAR_ARRAY_TO_STRING_P(array, num, pnum) ToString<char>(array, std::hex, num, pnum, 2)
