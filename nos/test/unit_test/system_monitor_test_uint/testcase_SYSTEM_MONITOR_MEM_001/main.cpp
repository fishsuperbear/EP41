...
protected:
    ...
    void SetUp() override {
        ...
        m_instance = std::make_unique<SystemMonitorMemMonitor>(configInfo.subFunction[SystemMonitorSubFunctionId::kMemMonitor]);
    }

    void TearDown() override {}

protected:
    std::unique_ptr<SystemMonitorMemMonitor> m_instance;
};

TEST_F(FuncTest, GetMemStatus) {
    MemInfo memStatus;
    int32_t res = m_instance->GetMemStatus(memStatus);
    EXPECT_TRUE((-1 != res) && (0 != res));
}

TEST_F(FuncTest, GetPidMemStatus) {
    std::vector<PidMemInfo> pidMem;
    int32_t res = m_instance->GetPidMemStatus(pidMem);
    EXPECT_TRUE((-1 != res) && (0 != res));
}

int main(int argc, char* argv[])
{
    hozon::netaos::log::InitLogging("system_monitor_mem_unit_test","utest",hozon::netaos::log::LogLevel::kOff,
