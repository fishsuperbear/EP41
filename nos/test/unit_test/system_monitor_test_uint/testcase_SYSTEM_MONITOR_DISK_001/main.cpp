...
protected:
    ...
    void SetUp() override {
        ...
        m_instance = std::make_unique<SystemMonitorDiskMonitor>(configInfo.subFunction[SystemMonitorSubFunctionId::kDiskMonitor]);
    }

    void TearDown() override {}

protected:
    std::unique_ptr<SystemMonitorDiskMonitor> m_instance;
};

TEST_F(FuncTest, GetDiskStatus) {
    std::vector<DiskInfo> diskStatus;
    int32_t res = m_instance->GetDiskStatus(diskStatus);
    EXPECT_TRUE((-1 != res) && (0 != res));
}

int main(int argc, char* argv[])
{
    hozon::netaos::log::InitLogging("system_monitor_disk_unit_test","utest",hozon::netaos::log::LogLevel::kOff,
