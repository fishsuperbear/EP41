...
protected:
    ...
    void SetUp() override {
        ...
        m_instance = std::make_unique<SystemMonitorVoltageMonitor>(configInfo.subFunction[SystemMonitorSubFunctionId::kVoltageMonitor]);
    }

    void TearDown() override {}

protected:
    std::unique_ptr<SystemMonitorVoltageMonitor> m_instance;
};

TEST_F(FuncTest, GetVoltage) {
    VoltageData voltage {0, 0};
    int32_t res = m_instance->GetVoltage(voltage);
    EXPECT_EQ(res, 0);
}

int main(int argc, char* argv[])
{
    hozon::netaos::log::InitLogging("system_monitor_voltage_unit_test","utest",hozon::netaos::log::LogLevel::kOff,
