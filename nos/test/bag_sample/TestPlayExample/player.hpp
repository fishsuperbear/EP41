...
public:
    explicit PlayerPublisher(
      std::shared_ptr<BagPublisher> pub)
    : publisher_(std::move(pub))
    {
      publish_func_ = std::bind(&BagPublisher::publish, publisher_, _1);
      // using std::placeholders::_1;
      // if (disable_loan_message || !publisher_->can_loan_messages()) {
      //   publish_func_ = std::bind(&BagPublisher::publish, publisher_, _1);
      // } else {
      //   publish_func_ = std::bind(&BagPublisher::publish_as_loaned_msg, publisher_, _1);
      // }
    }
    ...
    std::shared_ptr<BagPublisher> generic_publisher()
    {
      return publisher_;
    }

private:
    std::shared_ptr<BagPublisher> publisher_;
