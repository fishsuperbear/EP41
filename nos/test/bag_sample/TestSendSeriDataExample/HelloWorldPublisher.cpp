...
bool HelloWorldPublisher::init(bool use_env) {
    std::cout << "HelloWorldPublisher::init." << std::endl;
    // hello_.index(0);
    ...
    //std::cout << "init type_.register_type start." << std::endl;
    // type_.get()->auto_fill_type_information(true);
...
void HelloWorldPublisher::runThread(uint32_t samples,
...
void HelloWorldPublisher::run(uint32_t samples, uint32_t sleep) {
    ...
    reader_->open("/home/sw/work/netaos/output/examples/cpp/dds/bagtest/2023-04-14_15-11-11");
    std::cout << "reader_->open(/home/sw/work/netaos/output/examples/cpp/rtps/Registered/rtps_bag) ok" << std::endl;
    ...
    stop_ = false;
    //std::thread thread(&HelloWorldPublisher::runThread, this, samples, sleep, fid);
    std::thread thread(&HelloWorldPublisher::runThread, this, samples, sleep, reader_.get());
    if (samples == 0) {
...
bool HelloWorldPublisher::publish(rosbag2_cpp::Reader* reader_,  //FILE* fid,
                                  bool waitForListener) {

    if (listener_.firstConnected_ || !waitForListener || listener_.matched_ > 0) {
        std::cout << "HelloWorldPublisher::publish start.." << std::endl;
        ...
        std::memcpy(data.m_payload.data, rosbag_message->serialized_data->buffer, data.m_payload.length);
