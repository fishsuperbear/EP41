...
/**
 * @file RecorderPublisher.cpp
 *
 */

#include "RecorderPublisher.h"
...
RecorderPublisher::RecorderPublisher()
    : mp_participant(nullptr)
    , mp_publisher(nullptr)
{
}

bool RecorderPublisher::init()
{
    ...
    TypeSupport m_type(std::make_shared<types::DynamicPubSubType>(dyn_type));
    m_Hello = types::DynamicDataFactory::get_instance()->create_data(dyn_type);

    m_Hello->set_uint32_value(0, 0);
    m_Hello->set_string_value("Hello DDS Dynamic World", 1);
    ...
    topic_ = mp_participant->create_topic("HelloWorldTopic", "HelloWorld", TOPIC_QOS_DEFAULT);
    std::cout << "creat topic: " << topic_->get_name() << " " << topic_->get_type_name() << std::endl;
...
void RecorderPublisher::PubListener::on_publication_matched(
...
void RecorderPublisher::runThread(
        uint32_t samples,
        uint32_t sleep)
{
    if (samples == 0)
...
void RecorderPublisher::run(
        uint32_t samples,
        uint32_t sleep)
{
    stop = false;
    std::thread thread(&RecorderPublisher::runThread, this, samples, sleep);
    if (samples == 0)
    {
...
bool RecorderPublisher::publish(
