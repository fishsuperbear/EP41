...
#include "proto_image_executor.h"
...
ProtoCudaExecutor::ProtoCudaExecutor() {}
...
int32_t ProtoCudaExecutor::AlgInit() {
    ...
    NODE_LOG_INFO << "Init ProtoCudaExecutor.";
    RegistAlgProcessFunc("proto_cam", std::bind(&ProtoCudaExecutor::NVSCamProcess, this, std::placeholders::_1));
    RegistAlgProcessFunc("front_proto_cam", std::bind(&ProtoCudaExecutor::FrontNVSCamProcess, this, std::placeholders::_1));
    RegistAlgProcessFunc("avm_proto_cam", std::bind(&ProtoCudaExecutor::AVMNVSCamProcess, this, std::placeholders::_1));
    RegistAlgProcessFunc("side_proto_cam", std::bind(&ProtoCudaExecutor::SideNVSCamProcess, this, std::placeholders::_1));
    RegistAlgProcessFunc("dump_proto_cam", std::bind(&ProtoCudaExecutor::DumpNVSCamProcess, this, std::placeholders::_1));

    LiteRpc::GetInstance().RegisterServiceFunc("GetLatestData", std::bind(&ProtoCudaExecutor::GetLatestData, this, std::placeholders::_1));
    return 0;
}

void ProtoCudaExecutor::AlgRelease() {
    NODE_LOG_INFO << "Release ProtoCudaExecutor.";
...
int32_t ProtoCudaExecutor::DumpNVSCamProcess(Bundle* input) {
    cam_idx ++;

    std::shared_ptr<NvsImageCUDA> camera_0_ptr = std::static_pointer_cast<NvsImageCUDA>(input->GetOne("/soc/camera_0"));
    ...
    freq_checker.say("/soc/camera_0");
    std::shared_ptr<NvsImageCUDA> camera_1_ptr = std::static_pointer_cast<NvsImageCUDA>(input->GetOne("/soc/camera_1"));
    if (camera_1_ptr == nullptr) {
        NODE_LOG_INFO << "Fail to recv camera_1.";
        return -1;
    }
    freq_checker.say("/soc/camera_1");

    if (dump_file == true) {
        ImageDumpFile(std::string("camera_1"), cam_idx, camera_1_ptr);
    }

    std::shared_ptr<NvsImageCUDA> camera_3_ptr = std::static_pointer_cast<NvsImageCUDA>(input->GetOne("/soc/camera_3"));
    if (camera_3_ptr == nullptr) {
        NODE_LOG_INFO << "Fail to recv camera_3.";
        return -1;
    }
    freq_checker.say("/soc/camera_3");

    if (dump_file == true) {
        ImageDumpFile(std::string("camera_3"), cam_idx, camera_3_ptr);
    }

    std::shared_ptr<NvsImageCUDA> camera_4_ptr = std::static_pointer_cast<NvsImageCUDA>(input->GetOne("/soc/camera_4"));
    if (camera_4_ptr == nullptr) {
        NODE_LOG_INFO << "Fail to recv camera_4.";
        return -1;
    }
    freq_checker.say("/soc/camera_4");

    if (dump_file == true) {
        ImageDumpFile(std::string("camera_4"), cam_idx, camera_4_ptr);
    }

    std::shared_ptr<NvsImageCUDA> camera_5_ptr = std::static_pointer_cast<NvsImageCUDA>(input->GetOne("/soc/camera_5"));
    if (camera_5_ptr == nullptr) {
        NODE_LOG_INFO << "Fail to recv camera_5.";
        return -1;
    }
    freq_checker.say("/soc/camera_5");

    if (dump_file == true) {
        ImageDumpFile(std::string("camera_5"), cam_idx, camera_5_ptr);
    }

    std::shared_ptr<NvsImageCUDA> camera_6_ptr = std::static_pointer_cast<NvsImageCUDA>(input->GetOne("/soc/camera_6"));
    if (camera_6_ptr == nullptr) {
        NODE_LOG_INFO << "Fail to recv camera_6.";
        return -1;
    }
    freq_checker.say("/soc/camera_6");

    if (dump_file == true) {
        ImageDumpFile(std::string("camera_6"), cam_idx, camera_6_ptr);
    }

    std::shared_ptr<NvsImageCUDA> camera_7_ptr = std::static_pointer_cast<NvsImageCUDA>(input->GetOne("/soc/camera_7"));
    if (camera_7_ptr == nullptr) {
        NODE_LOG_INFO << "Fail to recv camera_7.";
        return -1;
    }
    freq_checker.say("/soc/camera_7");

    if (dump_file == true) {
        ImageDumpFile(std::string("camera_7"), cam_idx, camera_7_ptr);
    }

    std::shared_ptr<NvsImageCUDA> camera_8_ptr = std::static_pointer_cast<NvsImageCUDA>(input->GetOne("/soc/camera_8"));
    if (camera_8_ptr == nullptr) {
        NODE_LOG_INFO << "Fail to recv camera_8.";
        return -1;
    }
    freq_checker.say("/soc/camera_8");

    if (dump_file == true) {
        ImageDumpFile(std::string("camera_8"), cam_idx, camera_8_ptr);
    }

    std::shared_ptr<NvsImageCUDA> camera_9_ptr = std::static_pointer_cast<NvsImageCUDA>(input->GetOne("/soc/camera_9"));
    if (camera_9_ptr == nullptr) {
        NODE_LOG_INFO << "Fail to recv camera_9.";
        return -1;
    }
    freq_checker.say("/soc/camera_9");

    if (dump_file == true) {
        ImageDumpFile(std::string("camera_9"), cam_idx, camera_9_ptr);
    }

    std::shared_ptr<NvsImageCUDA> camera_10_ptr = std::static_pointer_cast<NvsImageCUDA>(input->GetOne("/soc/camera_10"));
    if (camera_10_ptr == nullptr) {
        NODE_LOG_INFO << "Fail to recv camera_10.";
        return -1;
    }
    freq_checker.say("/soc/camera_10");

    if (dump_file == true) {
        ImageDumpFile(std::string("camera_10"), cam_idx, camera_10_ptr);
    }

    std::shared_ptr<NvsImageCUDA> camera_11_ptr = std::static_pointer_cast<NvsImageCUDA>(input->GetOne("/soc/camera_11"));
    if (camera_11_ptr == nullptr) {
        NODE_LOG_INFO << "Fail to recv camera_11.";
        return -1;
    }
    freq_checker.say("/soc/camera_11");
...
int32_t ProtoCudaExecutor::NVSCamProcess(Bundle* input) {
    cam_idx ++;
...
int32_t ProtoCudaExecutor::FrontNVSCamProcess(Bundle* input) {
    std::shared_ptr<NvsImageCUDA> camera_0_ptr = std::static_pointer_cast<NvsImageCUDA>(input->GetOne("/soc/camera_0"));
    if (camera_0_ptr == nullptr) {
        NODE_LOG_INFO << "Fail to recv camera_0.";
        return -1;
...
int32_t ProtoCudaExecutor::AVMNVSCamProcess(Bundle* input) {
    std::shared_ptr<NvsImageCUDA> camera_8_ptr = std::static_pointer_cast<NvsImageCUDA>(input->GetOne("/soc/camera_8"));
    if (camera_8_ptr == nullptr) {
        NODE_LOG_INFO << "Fail to recv camera_8.";
        return -1;
...
int32_t ProtoCudaExecutor::SideNVSCamProcess(Bundle* input) {
    std::shared_ptr<NvsImageCUDA> camera_4_ptr = std::static_pointer_cast<NvsImageCUDA>(input->GetOne("/soc/camera_4"));
    if (camera_4_ptr == nullptr) {
        NODE_LOG_INFO << "Fail to recv camera_4.";
        return -1;
...
void ProtoCudaExecutor::WriteFile(const std::string& name, uint8_t* data, uint32_t size) {
...
void ProtoCudaExecutor::ImageDumpFile(const std::string& file_name, int index, std::shared_ptr<NvsImageCUDA> packet) {
    if (index % 10 != 0) {
        return ;
    }
...
int32_t ProtoCudaExecutor::Workflow1(Bundle* input) {
    static int64_t i = 0;
    ++i;
...
int32_t ProtoCudaExecutor::Workflow2(Bundle* input) {
...
int32_t ProtoCudaExecutor::Workflow3(Bundle* input) {
...
/* 接收Struct 数据类型的Topic，
 */
int32_t ProtoCudaExecutor::ReceiveStructTopic(Bundle* input) {
...
*/
int32_t ProtoCudaExecutor::ReceiveCmTopic(Bundle* input) {
...
int32_t ProtoCudaExecutor::FreeDataTopic(Bundle* input) {
    Writer _writer;
...
int32_t ProtoCudaExecutor::GetLatestData(BaseDataTypePtr& ptr) {
    ptr = _lastet_data;
