...
uint8_t
IsoTP::isotp_ms_to_st_min(uint8_t ms) {
    uint8_t st_min;
...
/* st_min to msec  */
uint8_t
IsoTP::isotp_st_min_to_ms(uint8_t st_min) {
    uint8_t ms;
...
int
IsoTP::isotp_send_flow_control(IsoTpLink* link, uint8_t flow_status, uint8_t block_size, uint8_t st_min_ms) {
    ...
    ret = isotp_user_send_can_callback_(link->send_arbitration_id, message.as.data_array.ptr, sizeof(message));
#else    
    ret = isotp_user_send_can_callback_(link->send_arbitration_id,
...
int
IsoTP::isotp_send_single_frame(IsoTpLink* link, uint32_t id) {
    ...
    ret = isotp_user_send_can_callback_(id, message.as.data_array.ptr, sizeof(message));
#else
    ret = isotp_user_send_can_callback_(id,
            message.as.data_array.ptr,
            link->send_size + 1);
#endif
...
int
IsoTP::isotp_send_first_frame(IsoTpLink* link, uint32_t id) {
...
int
IsoTP::isotp_send_consecutive_frame(IsoTpLink* link) {
...
int
IsoTP::isotp_receive_single_frame(IsoTpLink *link, IsoTpCanMessage *message, uint8_t len) {
    /* check data length */
    if ((0 == message->as.single_frame.SF_DL) || (message->as.single_frame.SF_DL > (len - 1))) {
        debug_callback_("Single-frame length too small.");
...
int
IsoTP::isotp_receive_first_frame(IsoTpLink *link, IsoTpCanMessage *message, uint8_t len) {
    uint16_t payload_length;

    if (8 != len) {
        debug_callback_("First frame should be 8 bytes in length.");
        return ISOTP_RET_LENGTH;
    }
    ...
    if (payload_length <= 7) {
        debug_callback_("Should not use multiple frame transmission.");
        return ISOTP_RET_LENGTH;
    }
    
    if (payload_length > link->receive_buf_size) {
        debug_callback_("Multi-frame response too large for receiving buffer.");
...
int
IsoTP::isotp_receive_consecutive_frame(IsoTpLink *link, IsoTpCanMessage *message, uint8_t len) {
    ...
    if (remaining_bytes > len - 1) {
        debug_callback_("Consecutive frame too short.");
        return ISOTP_RET_LENGTH;
    }
...
int
IsoTP::isotp_receive_flow_control_frame(IsoTpLink *link, IsoTpCanMessage *message, uint8_t len) {
    /* check message length */
    if (len < 3) {
        debug_callback_("Flow control frame too short.");
        return ISOTP_RET_LENGTH;
    }
...
int
IsoTP::isotp_send(IsoTpLink *link, const uint8_t payload[], uint16_t size) {
    return isotp_send_with_id(link, link->send_arbitration_id, payload, size);
}

int
IsoTP::isotp_send_with_id(IsoTpLink *link, uint32_t id, const uint8_t payload[], uint16_t size) {
    int ret;

    if (link == 0x0) {
        debug_callback_("Link is null!");
        return ISOTP_RET_ERROR;
    }

    if (size > link->send_buf_size) {
        debug_callback_("Message size too large. Increase ISO_TP_MAX_MESSAGE_SIZE to set a larger buffer\n");
        char message[128];
    ...
    if (ISOTP_SEND_STATUS_INPROGRESS == link->send_status) {
        debug_callback_("Abort previous message, transmission in progress.\n");
    ...
    if (link->send_size < 8) {
        ...
        if (ISOTP_RET_OK == ret) {
            ...
            link->send_timer_st = isotp_user_get_ms_callback_();
            link->send_timer_bs = isotp_user_get_ms_callback_() + ISO_TP_DEFAULT_RESPONSE_TIMEOUT;
...
void
IsoTP::isotp_on_can_message(IsoTpLink *link, uint8_t *data, uint8_t len) {
    IsoTpCanMessage message;
    int ret;
    ...
    switch (message.as.common.type) {
        ...
        case ISOTP_PCI_TYPE_FIRST_FRAME: {
            ...
            if (ISOTP_RET_OK == ret) {
                ...
                /* refresh timer cs */
                link->receive_timer_cr = isotp_user_get_ms_callback_() + ISO_TP_DEFAULT_RESPONSE_TIMEOUT;
            }
...
int
IsoTP::isotp_receive(IsoTpLink *link, uint8_t *payload, const uint16_t payload_size, uint16_t *out_size) {
    uint16_t copylen;
...
void
IsoTP::isotp_init_link(IsoTpLink *link, uint32_t sendid, uint8_t *sendbuf, uint16_t sendbufsize, uint8_t *recvbuf, uint16_t recvbufsize,
    std::function<void(const char* message)> debug_callback,
    std::function<int(const uint32_t arbitration_id,const uint8_t* data,const uint8_t size)> isotp_user_send_can_callback,
    std::function<uint32_t(void)> isotp_user_get_ms_callback
)
{
    debug_callback_ = debug_callback;
    isotp_user_send_can_callback_ = isotp_user_send_can_callback;
    isotp_user_get_ms_callback_ = isotp_user_get_ms_callback;
...
void
IsoTP::isotp_poll(IsoTpLink *link) {
    int ret;
    ...
    if (ISOTP_SEND_STATUS_INPROGRESS == link->send_status) {
        ...
        (0 == link->send_st_min || (0 != link->send_st_min && IsoTpTimeAfter(isotp_user_get_ms_callback_(), link->send_timer_st)))) {
            ...
            if (ISOTP_RET_OK == ret) {
                ...
                link->send_timer_bs = isotp_user_get_ms_callback_() + ISO_TP_DEFAULT_RESPONSE_TIMEOUT;
                link->send_timer_st = isotp_user_get_ms_callback_() + link->send_st_min;
        ...
        /* check timeout */
        if (IsoTpTimeAfter(isotp_user_get_ms_callback_(), link->send_timer_bs)) {
    ...
    if (ISOTP_RECEIVE_STATUS_INPROGRESS == link->receive_status) {
        ...
        /* check timeout */
        if (IsoTpTimeAfter(isotp_user_get_ms_callback_(), link->receive_timer_cr)) {
