...
private:
    ...
    int isotp_send_flow_control(IsoTpLink* link, uint8_t flow_status, uint8_t block_size, uint8_t st_min_ms);
    int isotp_send_single_frame(IsoTpLink* link, uint32_t id);
    int isotp_send_first_frame(IsoTpLink* link, uint32_t id);
    int isotp_send_consecutive_frame(IsoTpLink* link);
    int isotp_receive_single_frame(IsoTpLink *link, IsoTpCanMessage *message, uint8_t len);
    int isotp_receive_first_frame(IsoTpLink *link, IsoTpCanMessage *message, uint8_t len);
    int isotp_receive_consecutive_frame(IsoTpLink *link, IsoTpCanMessage *message, uint8_t len);
    int isotp_receive_flow_control_frame(IsoTpLink *link, IsoTpCanMessage *message, uint8_t len);


    std::function<void(const char* message)> debug_callback_;
    std::function<int(const uint32_t arbitration_id,const uint8_t* data,const uint8_t size)> isotp_user_send_can_callback_;
    std::function<uint32_t(void)> isotp_user_get_ms_callback_;
};
