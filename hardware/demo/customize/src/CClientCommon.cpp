...
EventStatus CClientCommon::HandleEvents(void)
{
    ...
    switch (event) {
        ...
        case NvSciStreamEventType_PacketReady:
            printf("HandleEvent, received NvSciStreamEventType_PacketReady.\n");
            status = HandlePayload();
            break;

        case NvSciStreamEventType_Error:
            printf("HandleEvent, received NvSciStreamEventType_Error.\n");
        ...
        case NvSciStreamEventType_Disconnected:
            printf("HandleEvent, received NvSciStreamEventType_Disconnected:\n");
            status = NVSIPL_STATUS_ERROR;
            break;
 ...
 */
SIPLStatus CClientCommon::MapDataBuffer(uint32_t packetIndex, NvSciBufObj bufObj)
{
    NvSciBufObjGetCpuPtr(bufObj, (void**)&m_metaPtrs[packetIndex]);
    return NVSIPL_STATUS_OK;
}
...
void CClientCommon::SetPacketElementsInfo(const std::vector<ElementInfo> &elemsInfo)
{
    for (auto &elem : elemsInfo) {
        if (elem.userType != ELEMENT_TYPE_UNDEFINED) {
            m_elemsInfo.push_back(elem);
        }
    }
}
...
SIPLStatus CClientCommon::HandleElemSupport(NvSciBufModule bufModule)
{
    ...
    for (uint32_t i = 0U; i < m_elemsInfo.size(); i++) {
        ...
        auto sciErr = NvSciBufAttrListCreate(bufModule, &bufAttrLists);
        /* printf("get elems size=%d,type = %d,%p \n",m_elemsInfo.size(),m_elemsInfo[i].userType,&m_bufAttrLists[i]); */
        ...
        printf("get elems size=%d,type = %d,%d,%p \n",m_elemsInfo.size(),m_elemsInfo[i].userType,ELEMENT_TYPE_ICP_RAW,&m_bufAttrLists[i]);
        /* sciErr = NvSciStreamBlockElementAttrSet(m_handle, ELEMENT_TYPE_ICP_RAW, m_bufAttrLists[i]); */
        sciErr = NvSciStreamBlockElementAttrSet(m_handle, i, bufAttrLists);
...
SIPLStatus CClientCommon::HandlePacketCreate(void)
{
    ...
    m_numPacket++;
    printf("Received PacketCreate from pool, m_numPackets: %u.\n", m_numPacket);
    ...
    for (uint32_t i = 0U; i < 1; i++) {
        ...
        /* if (m_elemsInfo[i].userType != ELEMENT_TYPE_METADATA) { */
            ...
            status = MapDataBuffer(packetIndex, bufObj);
    ...
    PCHK_NVSCISTATUS_AND_RETURN(sciErr, "Inform pool of packet status");
    printf("Set packet status success, cookie: %lu.\n", cookie);
...
SIPLStatus CClientCommon::HandleSyncExport(void)
{
    ...
    for (uint32_t i = 0U; i < 1; i++) {
        ...
        NvSciStreamBlockElementSignalObjSet(m_handle, 0, signalSyncObj);
...
SIPLStatus CClientCommon::CollectWaiterAttrList(uint32_t elementId, std::vector<NvSciSyncAttrList> &unreconciled)
{
    ...
    if (attrList != nullptr) {
        ...
        printf("Get null waiter attr, element id %u\n", elementId);
    }
...
{
    printf("Set signalObjSet for user type %d \n", m_elemsInfo[elementId].userType);
