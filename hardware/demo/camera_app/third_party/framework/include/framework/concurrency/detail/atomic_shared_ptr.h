...
#include "framework/concurrency/detail/counted_detail.h"
#include "framework/concurrency/detail/packed_ptr.h"
...
class atomic_shared_ptr {
    ...
    using CountedDetail = netaos::framework::concurrency::detail::CountedDetail;
    using BasePtr = typename CountedDetail::counted_base;
    using PackedPtr = netaos::framework::concurrency::detail::PackedPtr<BasePtr>;
...
public:
    ...
    bool compare_exchange_weak(
        ...
        return compare_exchange_weak(
            expected, n, mo, folly::detail::default_failure_memory_order(mo));
    }
    ...
    bool compare_exchange_weak(
        ...
        std::memory_order mo = std::memory_order_seq_cst) noexcept {
        return compare_exchange_weak(
            expected, desired, mo, folly::detail::default_failure_memory_order(mo));
    }
    ...
    PackedPtr takeOwnedBase(std::memory_order order) const noexcept {
        ...
        while (true) {
            ...
            if (get_local_count(newlocal) + 1 > EXTERNAL_OFFSET) {
                ...
                newlocal.set_extra(newlocal.extra() + 1);
    ...
    void putOwnedBase(BasePtr* p, unsigned int count, std::memory_order mo) const noexcept {
        ...
        while (true) {
            ...
            if (get_local_count(local) > count) {
                newlocal.set_extra(local.extra() - count);
            } else {
