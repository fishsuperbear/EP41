...
#include "framework/blocker/blocker.h"
#include "framework/common/global_data.h"
#include "framework/croutine/routine_factory.h"
#include "framework/data/data_visitor.h"
#include "framework/node/reader_base.h"
...
const uint32_t DEFAULT_PENDING_QUEUE_SIZE = 10;
...
bool Reader<MessageT>::Init() {
  ...
  if (reader_func_ != nullptr) {
    func = [this](const std::shared_ptr<MessageT>& msg) {
      this->Enqueue(msg);
#ifdef ENABLE_TRACE
      TRACE_ENTRY_EXT(FrameworkTraceModule::READERCALLBACK, m_context);
      TRACE_RECEIVE_MSG(msg);
      TRACE_CALLBACK_INFO(this->reader_func_);
      TRACE_SYS_PERF();
#endif
