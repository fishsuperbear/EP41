...
EventStatus CPoolManager::HandleEvents(void)
{
    ...
    switch (event) {
        ...
        case NvSciStreamEventType_PacketStatus:
            if (++m_numPacketReady < MAX_NUM_PACKETS) {
                break;
            }
...
SIPLStatus CPoolManager::HandlePacketsStatus(void)
{
    ...
    /* Check each packet */
    for (uint32_t p = 0; p < MAX_NUM_PACKETS; ++p) {
...
SIPLStatus CPoolManager::ReconcileAndAllocBuffers(NvSciBufAttrList& bufAttrList,
                                                           std::vector<NvSciBufObj>& vBufObjs)
{
    NvSciBufObj obj = nullptr;
