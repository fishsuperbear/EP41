...
SIPLStatus CEncConsumer::SetDataBufAttrList(void) {
    ...
    status = NvMediaIEPFillNvSciBufAttrList(NVMEDIA_ENCODER_INSTANCE_0, m_bufAttrLists[DATA_ELEMENT_INDEX]);
    ...
    err = NvSciBufAttrListSetAttrs(m_bufAttrLists[DATA_ELEMENT_INDEX], attributes, sizeof(attributes) / sizeof(NvSciBufAttrKeyValuePair));
    PCHK_NVSCISTATUS_AND_RETURN(err, "NvSciBufAttrListSetAttrs");
...
SIPLStatus CEncConsumer::MapDataBuffer(uint32_t packetIndex)
{
    if (m_pNvMIEP.get()) {
        ...
        m_pSciBufObjs[packetIndex] = m_packets[packetIndex].dataObj;
...
SIPLStatus CEncConsumer::EncodeOneFrame(NvSciBufObj pSciBufObj, uint8_t **ppOutputBuffer, size_t *pNumBytes, NvSciSyncFence *pPostfence)
{
