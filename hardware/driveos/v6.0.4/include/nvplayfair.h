...
#define ARCH_IS_X86                     (defined(__i386__) || defined(__amd64__))
#define ARCH_IS_NOT_X86                 !(ARCH_IS_X86)
...
#ifdef NVP_LOG_USE_NVOS
#include <nvos.h>
#define SLOG_MAX_LINE_LENGTH            (1024U)
#endif /* NVP_LOG_USE_NVOS */
...
#define LL_OFF                          (0U)
#define LL_ERROR                        (1U)
#define LL_WARNING                      (2U)
#define LL_REPORT                       (3U)
#define LL_STATUS                       (4U)
#define LL_DEBUG                        (5U)

#ifndef NVP_LOG_LEVEL
#define NVP_LOG_LEVEL                   (LL_STATUS)
#endif

#ifdef NVP_LOG_USE_NVOS
#define NVP_LOG_MSG(lvl, ...) {                                                     \
    if (NVP_LOG_LEVEL >= lvl) {                                                     \
        char buf[SLOG_MAX_LINE_LENGTH];                                             \
        snprintf(buf, SLOG_MAX_LINE_LENGTH, __VA_ARGS__);                           \
        NvOsDebugPrintStr(NVOS_SLOG_CODE_NVOS, NVOS_LOG_SEVERITY_INFO, buf);        \
    }                                                                               \
}
#else
#define NVP_LOG_MSG(lvl, ...) {                                                     \
    if (NVP_LOG_LEVEL >= lvl) {                                                     \
        if (lvl == LL_STATUS || lvl == LL_REPORT) {                                 \
...
#define NVP_CHECKERR_EXIT(e) {                                                      \
    if (e != NVP_PASS) {                                                            \
        NVP_LOG_MSG(LL_ERROR, "%s, %s:%d, NvPlayfair Error: %s\n",                  \
...
static inline uint64_t NvpGetTimeMark(void)
{
    ...
    if (ret != 0) {
        NVP_LOG_MSG(LL_ERROR, "%s, %s:%d, NvPlayfair Error clock_gettime %d\n",
...
DISABLE_FUNC(NvpConstructPerfData(NvpPerfData_t *perfData,
        uint64_t numOfSamples, char const *filename));
