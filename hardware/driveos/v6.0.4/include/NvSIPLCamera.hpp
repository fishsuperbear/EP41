...
class INvSIPLCamera
{
 public:
     ...
     *
     * This method sets a camera pipeline configuration. It must be called after
     ...
     *
     * This method must be called for every pipeline with ISP output enabled. It must be
     ...
     *
     * This method must be called after @ref SetPipelineCfg() but before RegisterImageGroups() (if
     * being used before deprecation) and @ref RegisterImages.
     *
     * @returns::SIPLStatus the completion status of the operation.
     *
     * @usage
     ...
     *
     * This method must be called after @ref Init() and before @ref Start().
     *
     * @param[in] index The ID of the sensor.
     ...
     * module.
     *
     * This method can only be called after @ref Init() but before
     * @ref Start().
     *
     ...
     *   - De-Init: No
     */
    virtual SIPLStatus ReadEEPROMData(const uint32_t index,
     ...
     *
     * This method must be called after @ref Init() and before @ref Start(). Additionally, if ISP
     * output is enabled it must be called before @ref RegisterAutoControlPlugin(). ISP Images are
     * to be registered before ICP Images.
     *
     * @param[in] index The ID of the sensor.
    ...
    virtual SIPLStatus RegisterImages(uint32_t const index,
                                      INvSIPLClient::ConsumerDesc::OutputType const outType,
                                      const std::vector<NvSciBufObj> &images) = 0;
     ...
     *
     * This method must be called after @ref RegisterImageGroups(), (if ISP output is enabled)
     * @ref RegisterImages(), and (if using NvSci) @ref RegisterNvSciSyncObj().
     *
     * @returns::SIPLStatus the completion status of the operation.
     *
     * @usage
     ...
     *   - De-Init: No
     */
    virtual SIPLStatus GetMaxErrorSize(const uint32_t devBlkIndex,
     ...
     *   - De-Init: No
     */
    virtual SIPLStatus GetErrorGPIOEventInfo(const uint32_t devBlkIndex,
     ...
     *
     * This method must be called after @ref Init() and before @ref Stop().
     *
     * @param[in]  devBlkIndex              Index of the device block associated with
     ...
     *   - De-Init: No
     */
    virtual SIPLStatus GetDeserializerErrorInfo(const uint32_t devBlkIndex,
     ...
     *   - De-Init: No
     */
    virtual SIPLStatus GetModuleErrorInfo(const uint32_t index,
                                          SIPLErrorDetails * const serializerErrorInfo,
                                          SIPLErrorDetails * const sensorErrorInfo,
                                          const SIPLModuleErrorReadFlag errorsToRead
