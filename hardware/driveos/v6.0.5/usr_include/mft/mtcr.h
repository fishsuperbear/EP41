...
#define ToHandle(h) ((HANDLE)(h))
    ...
    /*
     * update gearbox device mfile with secondary address and address width.
     */
     ...
     * When device is I2C primary - overclock it
     */
...
#define MTCR_MFT_2_7_0
    ...
    MTCR_API int set_up_server_connection_dl(void** connection, int port);

    MTCR_API int set_up_client_connection_dl(void** connection, const char* host, int port);

    MTCR_API int read_ssl_dl(void** connection, char* buffer, int buffer_size);

    MTCR_API int write_ssl_dl(void** connection, char* buffer);

    MTCR_API int shut_down_ssl_dl(void** connection);

    MTCR_API int read_device_id(mfile* mf, u_int32_t* device_id);

    MTCR_API int mft_core_init_device(const char* device_name);
#ifdef __cplusplus
}
#endif
