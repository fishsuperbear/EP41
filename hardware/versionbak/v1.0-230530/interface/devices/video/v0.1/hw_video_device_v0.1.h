...
enum HW_VIDEO_REGDATACB_TYPE
{
	...
	HW_VIDEO_REGDATACB_TYPE_AVC,	//h264
	HW_VIDEO_REGDATACB_TYPE_HEVC,	//h265
...
enum HW_VIDEO_BUFFERFORMAT_MAINTYPE
{
	...
	HW_VIDEO_BUFFERFORMAT_MAINTYPE_AVC,
	HW_VIDEO_BUFFERFORMAT_MAINTYPE_HEVC,
...
enum HW_VIDEO_BUFFERFORMAT_SUBTYPE
{
	...
	// common yuv420 format (in nvidia platform it is equal to yuv420 pl)
	HW_VIDEO_BUFFERFORMAT_SUBTYPE_YUV420,
	// private yuv420 format (in nvidia platform it is equal to yuv420 bl)
	HW_VIDEO_BUFFERFORMAT_SUBTYPE_YUV420_PRIV,
	HW_VIDEO_BUFFERFORMAT_SUBTYPE_RGBA,

	HW_VIDEO_BUFFERFORMAT_SUBTYPE_AVC,
	HW_VIDEO_BUFFERFORMAT_SUBTYPE_HEVC,

	HW_VIDEO_BUFFERFORMAT_SUBTYPE_MAXADDONE,
	HW_VIDEO_BUFFERFORMAT_SUBTYPE_MAX = HW_VIDEO_BUFFERFORMAT_SUBTYPE_MAXADDONE - 1,
};

enum HW_VIDEO_REGCUDADATACB_IMGTYPE
{
	HW_VIDEO_REGCUDADATACB_IMGTYPE_MIN = 0,
	HW_VIDEO_REGCUDADATACB_IMGTYPE_MINMINUSONE = HW_VIDEO_REGCUDADATACB_IMGTYPE_MIN - 1,

	// rgb888 sub type
	HW_VIDEO_REGCUDADATACB_IMGTYPE_RGB888_NHWC_RGB,
	// rgb888 sub type, the common rgb888 sub type
	HW_VIDEO_REGCUDADATACB_IMGTYPE_RGB888_NHWC_BGR,
	// rgb888 sub type
	HW_VIDEO_REGCUDADATACB_IMGTYPE_RGB888_NCHW_RGB,
	// rgb888 sub type
	HW_VIDEO_REGCUDADATACB_IMGTYPE_RGB888_NCHW_BGR,
	// rgb888 sub type
	HW_VIDEO_REGCUDADATACB_IMGTYPE_RGB888_NCHW16_RGB,
	// rgb888 sub type
	HW_VIDEO_REGCUDADATACB_IMGTYPE_RGB888_NCHW16_BGR,
	// yuv420 sub type, the common yuv420 sub type
	HW_VIDEO_REGCUDADATACB_IMGTYPE_NV12,
	// yuv420 sub type
	HW_VIDEO_REGCUDADATACB_IMGTYPE_NV21,
	// yuv422 sub type, the common yuv422 sub type
	HW_VIDEO_REGCUDADATACB_IMGTYPE_YUYV,
	// yuv422 sub type
	HW_VIDEO_REGCUDADATACB_IMGTYPE_YVYU,
	// yuv422 sub type
	HW_VIDEO_REGCUDADATACB_IMGTYPE_VYUY,

	HW_VIDEO_REGCUDADATACB_IMGTYPE_MAXADDONE,
	HW_VIDEO_REGCUDADATACB_IMGTYPE_MAX = HW_VIDEO_REGCUDADATACB_IMGTYPE_MAXADDONE - 1,
};

enum HW_VIDEO_REGCUDADATACB_INTERPOLATION
{
	// Nearest cost time less but effect may not be so good
	HW_VIDEO_REGCUDADATACB_INTERPOLATION_NEAREST,
	// Bilinear cost time but effect may be good
	HW_VIDEO_REGCUDADATACB_INTERPOLATION_BILINEAR,
};

enum HW_VIDEO_INPUT_TYPE
{
	HW_VIDEO_INPUT_TYPE_RAW12 = 0,
	HW_VIDEO_INPUT_TYPE_YUV422,
};
...
typedef struct hw_video_buffertimeinfo_t
{
    u64									framecapturetsc;
} hw_video_buffertimeinfo_t;
/*
* SHM_BUF
* -----------***************************
* |-SHM_HEAD-|---------imagebuffer------|
*/
...
#define SOCKET_NAME "/tmp/.cam_hal_enc_%d_%d"
static int _consumer_ser_key[4][4] = {
    {SHM_KEY_BLOCK0_SENSOR0, SHM_KEY_BLOCK0_SENSOR1, SHM_KEY_BLOCK0_SENSOR2, SHM_KEY_BLOCK0_SENSOR3},
    {SHM_KEY_BLOCK1_SENSOR0, SHM_KEY_BLOCK1_SENSOR1, SHM_KEY_BLOCK1_SENSOR2, SHM_KEY_BLOCK1_SENSOR3},
    {SHM_KEY_BLOCK2_SENSOR0, SHM_KEY_BLOCK2_SENSOR1, SHM_KEY_BLOCK2_SENSOR2, SHM_KEY_BLOCK2_SENSOR3},
    {0, 0, 0, 0}
};

typedef struct hw_video_shmimageheader_t {
    u32 magic_number;      // image type
    u32 width;             // image wigth
    u32 height;            // image height
    u64 timestamp;         // timestamp
    u32 reserved[4];       // Reserved fields are used to expand the header information in the future
} hw_video_shmimageheader_t;

/*
* The buffer information is retrieved to input parameter when datacb function is called.
* Used by all HW_VIDEO_REGDATACB_TYPE except cuda.
*/
typedef struct hw_video_bufferinfo_t
{
	...
	*/
	...
	// the custom height
	u32									height;
	// the output buffer width stride
	u32									stride;
	u32									rotatedegrees;
...
typedef struct hw_video_cudayuvbufferinfo_t
{
	HW_VIDEO_REGCUDADATACB_IMGTYPE		imgtype;
	void*								plumabuff;
	u32									lumabuffsize;
	void*								pchromabuff;
	u32									chromabuffsize;
...
} hw_video_cudayuvbufferinfo_t;

typedef struct hw_video_cudargbbufferinfo_t
{
	HW_VIDEO_REGCUDADATACB_IMGTYPE		imgtype;
	void*								pbuff;
	u32									buffsize;
} hw_video_cudargbbufferinfo_t;

typedef struct hw_video_cudagpubufferinfo_t
{
	HW_VIDEO_REGCUDADATACB_IMGTYPE		imgtype;
	// valid only when imgtype is yuv
	struct hw_video_cudayuvbufferinfo_t	yuvinfo;
	// valid only when imgtype is rgb
	struct hw_video_cudargbbufferinfo_t	rgbinfo;
} hw_video_cudagpubufferinfo_t;

/*
* The cuda buffer information is retrieved to input parameter when cuda datacb function is called.
*/
typedef struct hw_video_cudabufferinfo_t
{
	/*
	* The time we receive the frame.
	* Currently, we use the monotonic system time.
	*/
	...
	HW_VIDEO_SENSORPIPELINE_OUTPUTTYPE	outputtype;
	HW_VIDEO_REGCUDADATACB_IMGTYPE		imgtype;
	HW_VIDEO_REGCUDADATACB_INTERPOLATION	interpolation;
	// the origin capture width
	u32									capturewidth;
	// the origin capture height
	u32									captureheight;
	...
	u32									stride;
	u32									rotatedegrees;
	struct hw_video_cudagpubufferinfo_t	gpuinfo;
	// same as the input when you call pipeline_open
	u32									bsynccb;
	/*
	* 1 mean you need to free after use, 0 mean not
...
typedef struct hw_video_outputpipeline_ops_t
{
	...
	*/
	s32(*skiphandle)(struct hw_video_outputpipeline_ops_t* i_poutputpipeline_ops,
		hw_video_handleframe i_handleframe);
    /*
	* We can use the function to get the time info of the current i_handleframe frame.
	*/
    s32 (*getframetimeinfo)(struct hw_video_outputpipeline_ops_t* i_poutputpipeline_ops,
...
typedef struct hw_video_sensorpipeline_ops_t
{
	...
	*/
	...
	u32											blockindex; // valid when bused is 1
	u32											sensorindex; // valid when bused is 1
	HW_VIDEO_INPUT_TYPE							inputType; // camera input type
...
* It is a callback function.
* The buffer information is set by hal and the user use it to do the handling operation.
* The buffer information i_pcudabufferinfo instance is not valid when the callback function return.
* The i_pcudabufferinfo is the buffer information of the current handling cuda frame.
*/
typedef void(*hw_video_sensorpipelinecudadatacb)(struct hw_video_cudabufferinfo_t* i_pcudabufferinfo);

typedef struct hw_video_sensorpipelinecudasdatacbconfig_t
{
	HW_VIDEO_REGCUDADATACB_IMGTYPE				imgtype;
	HW_VIDEO_REGCUDADATACB_INTERPOLATION		interpolation;
} hw_video_sensorpipelinecudasdatacbconfig_t;

typedef struct hw_video_sensorpipelinedatacbconfig_t
{
	// 0 or 1
	u32											bused;
	HW_VIDEO_REGDATACB_TYPE						type;
	// all HW_VIDEO_REGDATACB_TYPE type use it
	...
	u32											rotatedegrees;
	/*
	* Valid when HW_VIDEO_REGDATACB_TYPE type is not cuda.
	*/
	hw_video_sensorpipelinedatacb				cb;
	/*
	* Valid only when HW_VIDEO_REGDATACB_TYPE type is cuda.
	*/
	struct hw_video_sensorpipelinecudasdatacbconfig_t	cudaconfig;
	/*
	* Valid only when HW_VIDEO_REGDATACB_TYPE type is cuda.
	*/
	hw_video_sensorpipelinecudadatacb			cudacb;
	/*
	* All HW_VIDEO_REGDATACB_TYPE type use it.
...
typedef struct hw_video_sensorpipelineconfig_t
{
    ...
    //consumer for enc/vic/common/cuda
    u32                                         enablevic=0;
    u32 										enableenc=0;
    u32										    enablecommon=0;	
    u32											enablecuda=0;
