...
extern "C" struct hw_nvmedia_groupc_module_t HAL_MODULE_INFO_SYM;
...
static u32 _parray_testframecount_yuv422[4] = { 0 };

static u32 _binitfile_yuv422 = 0;

static FILE* _parray_pfiletest_yuv422[4] = { nullptr };
...
static void handle_yuv422_buffer(struct hw_video_bufferinfo_t* i_pbufferinfo)
{
	...
	*/
	if (_parray_testframecount_yuv422[i_pbufferinfo->sensorindex] < 3)
	{
		fwrite(i_pbufferinfo->pbuff, i_pbufferinfo->size, 1U, _parray_pfiletest_yuv422[i_pbufferinfo->sensorindex]);
		_parray_testframecount_yuv422[i_pbufferinfo->sensorindex]++;
	}
}
...
int main()
{
    ...
    */
	...
	{
		filename = "testfile_" + std::to_string(sensori) + ".yuv422";
		remove(filename.c_str());
		_parray_pfiletest_yuv422[sensori] = fopen(filename.c_str(), "wb");
		if (!_parray_pfiletest_yuv422[sensori]) {
	...
	u32 blocki, numblocks, numsensors, outputi;
