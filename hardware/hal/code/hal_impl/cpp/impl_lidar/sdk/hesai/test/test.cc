...
void pcl_show(boost::shared_ptr<PPointCloud> cld)
{
    for (int i = 0; i < cld->size(); i++)
    {
        PPoint point = cld->at(i);
        pcl::PointXYZI p;
        p.x = point.x;
        p.y = point.y;
        p.z = point.z;
...
int main(int argc, char** argv) {
    ...
    spPandarSwiftSDK.reset(new PandarSwiftSDK(std::string("192.168.3.202"), std::string(""), 2369, 10110, std::string("PandarAT128"), \
