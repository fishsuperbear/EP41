...
#if (HW_NVMEDIA_PROJ == HW_NVMEDIA_PROJ_MULTIGROUP)
#include "hw_nvmedia_common_impl.h"
#include "hw_nvmedia_multigroup_impl.h"

extern "C" struct hw_nvmedia_multigroup_module_t HAL_MODULE_INFO_SYM;
struct hw_nvmedia_multigroup_module_t HAL_MODULE_INFO_SYM =
{
	.common = {
...
static HWNvmediaMultiGroupEnv		_env;
...
s32 hw_module_privapi_device_get(struct hw_module_t* i_pmodule, void* i_param, struct hw_device_t** io_ppdevice)
{
	...
	*/
	...
	{
		_env.pcontext = new HWNvmediaMultiGroupContext(pvideo);
	...
	hw_plat_mutex_unlock(&_env.mutex_context);
	HW_NVMEDIA_LOG_INFO("Has init nvmedia multigroup context!\r\n");
...
s32 hw_module_privapi_device_put(struct hw_module_t* i_pmodule, struct hw_device_t* i_pdevice)
{
	...
	HWNvmediaMultiGroupContext *pcontext = (HWNvmediaMultiGroupContext *)pvideo->priv;
	delete(pcontext);
	delete(pvideo);
	return 0;
}
