...
#if (HW_NVMEDIA_PROJ == HW_NVMEDIA_PROJ_MULTIIPC_CONSUMER_CUDA)
#include "hw_nvmedia_common_impl.h"
#include "hw_nvmedia_multiipc_consumer_cuda_impl.h"

extern "C" struct hw_nvmedia_multiipc_consumer_cuda_module_t HAL_MODULE_INFO_SYM;
struct hw_nvmedia_multiipc_consumer_cuda_module_t HAL_MODULE_INFO_SYM =
{
	.common = {
		...
		.module_id = HW_VIDEO_MODULEID_NVMEDIA_IPC_CONSUMER_CUDA,
		...
		.devmoduleid_magic = hw_video_moduleid_magic_get(HW_VIDEO_MODULEID_NVMEDIA_IPC_CONSUMER_CUDA),
		.description = "ipc_consumer_cuda block " HW_NVMEDIA_VERSION,
		.privdata = {
			.dso = NULL,
			.pvoid = NULL,
		},
...
static HWNvmediaMultiIpcConsumerCudaEnv		_env;
...
s32 hw_module_privapi_device_get(struct hw_module_t* i_pmodule, void* i_param, struct hw_device_t** io_ppdevice)
{
	...
	*/
	...
	{
		_env.pcontext = new HWNvmediaMultiIpcConsumerCudaContext(pvideo);
...
s32 hw_module_privapi_device_put(struct hw_module_t* i_pmodule, struct hw_device_t* i_pdevice)
{
	...
	HWNvmediaMultiIpcConsumerCudaContext* pcontext = (HWNvmediaMultiIpcConsumerCudaContext*)pvideo->priv;
	delete(pcontext);
	delete(pvideo);
	return 0;
}
