...
#if (HW_NVMEDIA_PROJ == HW_NVMEDIA_PROJ_GROUPB)
...
struct hw_nvmedia_groupb_module_t HAL_MODULE_INFO_SYM =
{
	.common = {
		...
		.module_id = HW_VIDEO_MODULEID_NVMEDIA_GROUPB,
		...
		.devmoduleid_magic = hw_video_moduleid_magic_get(HW_VIDEO_MODULEID_NVMEDIA_GROUPB),
		.description = "groupb block",
		.privdata = {
			.dso = NULL,
			.pvoid = NULL,
		},
...
static HWNvmediaGroupbEnv		_env;
...
s32 hw_module_privapi_device_get(struct hw_module_t* i_pmodule, void* i_param, struct hw_device_t** io_ppdevice)
{
	...
	*/
	...
	{
		_env.pcontext = new HWNvmediaGroupbContext(pvideo);
	...
	hw_plat_mutex_unlock(&_env.mutex_context);
	HW_NVMEDIA_LOG_INFO("Has init nvmedia groupb context!\r\n");
...
s32 hw_module_privapi_device_put(struct hw_module_t* i_pmodule, struct hw_device_t* i_pdevice)
{
	...
	HWNvmediaGroupbContext* pcontext = (HWNvmediaGroupbContext*)pvideo->priv;
	delete(pcontext);
	delete(pvideo);
	return 0;
}
