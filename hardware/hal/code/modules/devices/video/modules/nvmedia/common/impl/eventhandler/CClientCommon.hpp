...
#ifndef CCLIENTCOMMON_HPP
#define CCLIENTCOMMON_HPP
...
enum StreamPhase
{
    ...
    // TODO:: StreamPhase_Disconnected
    StreamPhase_UpperBound,
};
...
class CClientCommon : public CEventHandler
{
    public:
        ...
        SIPLStatus Init(NvSciBufModule bufModule, NvSciSyncModule syncModule, bool enableLateAttach = false);
        virtual SIPLStatus Deinit(void){ return NVSIPL_STATUS_OK; }
        void SetProfiler(HWNvmediaSensorOutputPipelineProfiler *pProfiler);
    ...
    protected:
        ...
        inline SIPLStatus GetIndexFromCookie(NvSciStreamCookie cookie, uint32_t &index)
        {
            if (cookie <= cookieBase) {
                PLOG_ERR("%s:invalid cookie assignment\r\n", m_name.c_str());
        ...
        inline ClientPacket* GetPacketByCookie(const NvSciStreamCookie& cookie)
        {
            ...
            PLOG_DBG("%s:GetPacketByCookie: packetId: %u\r\n", m_name.c_str(), id);
        ...
        HWNvmediaSensorOutputPipelineProfiler *m_pProfiler = nullptr;
        std::vector<ElementInfo> m_elemsInfo;
        bool m_isSyncAttrListSet = false;
        bool                     m_enableLateAttach = false;
        StreamPhase              m_streamPhase = StreamPhase_Initialization;

    public:
        SIPLStatus CollectWaiterAttrList(uint32_t elementId, std::vector<NvSciSyncAttrList> &unreconciled);
