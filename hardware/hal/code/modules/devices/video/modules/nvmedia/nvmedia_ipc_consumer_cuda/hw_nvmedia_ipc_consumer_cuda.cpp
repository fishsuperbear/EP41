...
#if (HW_NVMEDIA_PROJ == HW_NVMEDIA_PROJ_IPC_CONSUMER_CUDA)
#include "hw_nvmedia_common_impl.h"
#include "hw_nvmedia_ipc_consumer_cuda_impl.h"

extern "C" struct hw_nvmedia_ipc_consumer_cuda_module_t HAL_MODULE_INFO_SYM;
struct hw_nvmedia_ipc_consumer_cuda_module_t HAL_MODULE_INFO_SYM =
{
	.common = {
		...
		.description = "ipc_consumer_cuda block",
		.privdata = {
			.dso = NULL,
			.pvoid = NULL,
		},
...
static HWNvmediaIpcConsumerCudaEnv		_env;
...
s32 hw_module_privapi_device_get(struct hw_module_t* i_pmodule, void* i_param, struct hw_device_t** io_ppdevice)
{
	...
	*/
	...
	{
		_env.pcontext = new HWNvmediaIpcConsumerCudaContext(pvideo);
...
s32 hw_module_privapi_device_put(struct hw_module_t* i_pmodule, struct hw_device_t* i_pdevice)
{
	...
	HWNvmediaIpcConsumerCudaContext* pcontext = (HWNvmediaIpcConsumerCudaContext*)pvideo->priv;
	delete(pcontext);
	delete(pvideo);
	return 0;
}
