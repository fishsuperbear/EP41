...
SIPLStatus CClientCommon::Init(NvSciBufModule bufModule, NvSciSyncModule syncModule, bool enableLateAttach)
{
    m_sciBufModule = bufModule;
    m_sciSyncModule = syncModule;
    m_enableLateAttach = enableLateAttach;
...
void CClientCommon::SetProfiler(HWNvmediaSensorOutputPipelineProfiler *pProfiler)
{
...
EventStatus CClientCommon::HandleEvents(void)
{
    ...
    if (NvSciError_Success != sciErr) {
        if (NvSciError_Timeout == sciErr) {
            PLOG_WARN("%s:Event query, timed out.\r\n", m_name.c_str());
            return EVENT_STATUS_TIMED_OUT;
        }
        PLOG_ERR("%s:Event query, failed with error 0x%x", m_name.c_str(), sciErr);
    ...
    switch (event) {
        ...
        case NvSciStreamEventType_Elements:
            PLOG_DBG("%s:HandleEvent, received NvSciStreamEventType_Elements.\r\n", m_name.c_str());
            status = HandleElemSetting();
            break;
        case NvSciStreamEventType_PacketCreate:
            /* Handle creation of a new packet */
            PLOG_DBG("%s:HandleEvent, received NvSciStreamEventType_PacketCreate.\r\n", m_name.c_str());
            status = HandlePacketCreate();
            break;
        case NvSciStreamEventType_PacketsComplete:
            /* Handle packet complete*/
            PLOG_DBG("%s:HandleEvent, received NvSciStreamEventType_PacketsComplete.\r\n", m_name.c_str());
        ...
        case NvSciStreamEventType_PacketReady:
            PLOG_DBG("%s:HandleEvent, received NvSciStreamEventType_PacketReady.\r\n", m_name.c_str());
            status = HandlePayload();
            break;

        case NvSciStreamEventType_Error:
            PLOG_ERR("%s:HandleEvent, received NvSciStreamEventType_Error.\r\n", m_name.c_str());
            sciErr = NvSciStreamBlockErrorGet(m_handle, &sciStatus);
            if (NvSciError_Success != sciErr) {
                PLOG_ERR("%s:Failed to query the error event code 0x%x\r\n", m_name.c_str(), sciErr);
            } else {
                PLOG_ERR("%s:Received error event: 0x%x\r\n", m_name.c_str(), sciStatus);
            }
            status = NVSIPL_STATUS_ERROR;
            break;
        case NvSciStreamEventType_Disconnected:
            PLOG_WARN("%s:HandleEvent, received NvSciStreamEventType_Disconnected:\r\n", m_name.c_str());
            status = NVSIPL_STATUS_ERROR;
            break;

        default:
            PLOG_ERR("%s:Received unknown event 0x%x\r\n", m_name.c_str(), event);
            status = NVSIPL_STATUS_ERROR;
            break;
    }
    PLOG_DBG("%s:HandleEvent, status = %u\r\n", m_name.c_str(), status);
...
SIPLStatus CClientCommon::HandlePacketCreate(void)
{
    ...
    if (MAX_NUM_PACKETS <= m_numPacket) {
        PLOG_ERR("%s:Exceeded max packets\r\n", m_name.c_str());
    ...
    m_numPacket++;
    PLOG_DBG("%s:Received PacketCreate from pool, m_numPackets: %u.\r\n", m_name.c_str(), m_numPacket);
    ...
    for (uint32_t i = 0U; i < m_elemsInfo.size(); ++i) {
        ...
        if (NvSciError_Success != sciErr) {
            PLOG_ERR("%s:Failed (0x%x) to retrieve buffer (0x%lx)\r\n", m_name.c_str(), sciErr, packetHandle);
            return NVSIPL_STATUS_ERROR;
        }
    ...
    PCHK_NVSCISTATUS_AND_RETURN(sciErr, "Inform pool of packet status");
    PLOG_DBG("%s:Set packet status success, cookie: %u.\r\n", m_name.c_str());
...
SIPLStatus CClientCommon::HandleSyncExport(void)
{
    ...
    {
        printf("CClientCommon::HandleSyncExport m_elemsInfo userType: %d, isUsed: %d, hasSibling: %d\n", 
...
SIPLStatus CClientCommon::HandleSyncImport(void)
{
    ...
    for (uint32_t i = 0U; i < m_numWaitSyncObj; ++i) {
        ...
        if (NvSciError_Success != sciErr) {
            PLOG_ERR("%s:Failed (0x%x) to query sync obj from index %d\r\n", m_name.c_str(), sciErr, i);
            return NVSIPL_STATUS_ERROR;
        }
