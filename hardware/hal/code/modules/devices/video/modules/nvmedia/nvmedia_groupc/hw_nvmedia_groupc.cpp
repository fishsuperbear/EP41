...
#if (HW_NVMEDIA_PROJ == HW_NVMEDIA_PROJ_GROUPC)
...
struct hw_nvmedia_groupc_module_t HAL_MODULE_INFO_SYM =
{
	.common = {
		...
		.module_id = HW_VIDEO_MODULEID_NVMEDIA_GROUPC,
		...
		.devmoduleid_magic = hw_video_moduleid_magic_get(HW_VIDEO_MODULEID_NVMEDIA_GROUPC),
		.description = "groupc block",
		.privdata = {
			.dso = NULL,
			.pvoid = NULL,
		},
...
static HWNvmediaGroupcEnv		_env;
...
s32 hw_module_privapi_device_get(struct hw_module_t* i_pmodule, void* i_param, struct hw_device_t** io_ppdevice)
{
	...
	*/
	...
	{
		_env.pcontext = new HWNvmediaGroupcContext(pvideo);
	...
	hw_plat_mutex_unlock(&_env.mutex_context);
	HW_NVMEDIA_LOG_INFO("Has init nvmedia groupc context!\r\n");
...
s32 hw_module_privapi_device_put(struct hw_module_t* i_pmodule, struct hw_device_t* i_pdevice)
{
	...
	HWNvmediaGroupcContext* pcontext = (HWNvmediaGroupcContext*)pvideo->priv;
	delete(pcontext);
	delete(pvideo);
	return 0;
}
