...
{
public:
    CPoolManager(NvSciStreamBlock handle, uint32_t uSensor, uint32_t numPackets, std::shared_ptr<CAttributeProvider> attrProvider = nullptr);
    ~CPoolManager(void);

    SIPLStatus Init(bool enableLateAttach = false);
...
private:
    ...
    std::shared_ptr<CAttributeProvider> m_attrProvider;
