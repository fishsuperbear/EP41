...
#if (HW_NVMEDIA_PROJ == HW_NVMEDIA_PROJ_MULTIIPC_CONSUMER_ENC)
#include "hw_nvmedia_common_impl.h"
#include "hw_nvmedia_multiipc_consumer_enc_impl.h"

extern "C" struct hw_nvmedia_multiipc_consumer_enc_module_t HAL_MODULE_INFO_SYM;
struct hw_nvmedia_multiipc_consumer_enc_module_t HAL_MODULE_INFO_SYM =
    {
        .common = {
            ...
            .module_id = HW_VIDEO_MODULEID_NVMEDIA_IPC_CONSUMER_ENC,
            ...
            .devmoduleid_magic = hw_video_moduleid_magic_get(HW_VIDEO_MODULEID_NVMEDIA_IPC_CONSUMER_ENC),
            .description = "ipc_consumer_enc block " HW_NVMEDIA_VERSION,
            .privdata = {
                .dso = NULL,
                .pvoid = NULL,
            },
...
static HWNvmediaMultiIpcConsumerEncEnv _env;
...
s32 hw_module_privapi_device_get(struct hw_module_t* i_pmodule, void* i_param, struct hw_device_t** io_ppdevice) {
    ...
    if (_env.pcontext == NULL) {
        _env.pcontext = new HWNvmediaMultiIpcConsumerEncContext(pvideo);
...
s32 hw_module_privapi_device_put(struct hw_module_t* i_pmodule, struct hw_device_t* i_pdevice) {
    ...
    HWNvmediaMultiIpcConsumerEncContext* pcontext = (HWNvmediaMultiIpcConsumerEncContext*)pvideo->priv;
    delete (pcontext);
    delete (pvideo);
    return 0;
}

#elif (HW_NVMEDIA_PROJ == HW_NVMEDIA_PROJ_MULTIIPC_PRODUCER)
#include "hw_nvmedia_common_impl.h"
#include "hw_nvmedia_multiipc_consumer_enc_impl.h"

extern "C" struct hw_nvmedia_multiipc_consumer_enc_module_t HAL_MODULE_INFO_SYM_ENC;
struct hw_nvmedia_multiipc_consumer_enc_module_t HAL_MODULE_INFO_SYM_ENC =
    {
        .common = {
            .tag = HARDWARE_MODULE_TAG,
            .hal_api_version = HARDWARE_HAL_API_VERSION,
            ...
            .privapi = {
                .init = hw_module_privapi_init_enc,
                .check_device_api_version = hw_module_privapi_check_device_api_version_enc,
                .trigger_check_unmasklogleft = hw_module_privapi_trigger_check_unmasklogleft_enc,
                .device_get = hw_module_privapi_device_get_enc,
                .device_put = hw_module_privapi_device_put_enc,
            },
        },
        .nvmedia = {
            .driveosversiondesc = "6.0.6",
...
static HWNvmediaMultiIpcConsumerEncEnv _env_enc;

s32 hw_module_privapi_init_enc(void** io_ppvoid) {
    if (_env_enc.binit == 0) {
        HW_NVMEDIA_LOG_UNMASK("Run init of module[%s]nvmedia-driveosversiondesc[%s]!\r\n", HAL_MODULE_INFO_SYM_ENC.common.description,
                              HAL_MODULE_INFO_SYM_ENC.nvmedia.driveosversiondesc);
        /*
         * Set all of the global .data content the init value.
         */
...
s32 hw_module_privapi_check_device_api_version_enc(u32 i_device_api_version) {
    _env_enc.device_api_version = i_device_api_version;
    /*
     * The module currently only support HARDWARE_DEVICE_API_VERSION(0, 1).
     */
    if (i_device_api_version == HW_MAKEV_DEVICE_API_VERSION(0, 1)) {
        _env_enc.bsupport_device_api_version = 1;
        HW_NVMEDIA_LOG_UNMASK("The current module support the device_api_version[%x]!\r\n", _env_enc.device_api_version);
        return 0;
    } else {
        _env_enc.bsupport_device_api_version = 0;
        HW_NVMEDIA_LOG_UNMASK("The current module does not support the device_api_version[%x]!\r\n", _env_enc.device_api_version);
        return -1;
    }
}

s32 hw_module_privapi_trigger_check_unmasklogleft_enc() {
    /*
     * For future use.
     */
    return 0;
...
s32 hw_module_privapi_device_get_enc(struct hw_module_t* i_pmodule, void* i_param, struct hw_device_t** io_ppdevice) {
    /*
     * Inner use only. Only for nvmedia internal api currently.
     */
    hw_ret_s32 ret;
     ...
     * when call hw_module_get in the previous step.
     */
    if (HW_UNLIKELY(_env_enc.bsupport_device_api_version == 0)) {
        HW_NVMEDIA_LOG_UNMASK("bsupport_device_api_version is 0, so cannot get device!\r\n");
        return -1;
    }
    struct hw_video_t* pvideo = (struct hw_video_t*)malloc(sizeof(struct hw_video_t));
    pvideo->common.tag = HARDWARE_DEVICE_TAG;
    pvideo->common.device_api_version = _env_enc.device_api_version;
    pvideo->common.pmodule = (hw_module_t*)&HAL_MODULE_INFO_SYM_ENC;
    hw_nvmedia_setvideoops(pvideo);
    /*
     * Check get pcontext.
     */
    hw_plat_mutex_lock(&_env_enc.mutex_context);
    if (_env_enc.pcontext == NULL) {
        _env_enc.pcontext = new HWNvmediaMultiIpcConsumerEncContext(pvideo);
        ret = _env_enc.pcontext->Init();
        if (ret != 0) {
            HW_NVMEDIA_LOG_UNMASK("ContextPrepare fail!\r\n");
            delete (_env_enc.pcontext);
            hw_plat_mutex_unlock(&_env_enc.mutex_context);
            return -2;
        }
        _env_enc.refcount_pcontext = 0;
    }
    pvideo->priv = _env_enc.pcontext;
    _env_enc.refcount_pcontext++;
    hw_plat_mutex_unlock(&_env_enc.mutex_context);
    HW_NVMEDIA_LOG_INFO("Has init nvmedia groupa context!\r\n");
    HW_NVMEDIA_LOG_INFO("The current drive os version[%s]\r\n", HAL_MODULE_INFO_SYM_ENC.nvmedia.driveosversiondesc);
    *io_ppdevice = (struct hw_device_t*)pvideo;
...
s32 hw_module_privapi_device_put_enc(struct hw_module_t* i_pmodule, struct hw_device_t* i_pdevice) {
    struct hw_video_t* pvideo = (struct hw_video_t*)i_pdevice;
    if (HW_UNLIKELY(pvideo->common.tag != HARDWARE_DEVICE_TAG)) {
        HW_NVMEDIA_LOG_UNMASK("unexpected video device tag[%x], should be HARDWARE_DEVICE_TAG[%x]\r\n", pvideo->common.tag, HARDWARE_DEVICE_TAG);
        return -1;
