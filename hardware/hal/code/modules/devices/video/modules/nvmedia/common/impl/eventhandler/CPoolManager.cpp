...
CPoolManager::CPoolManager(NvSciStreamBlock handle, uint32_t uSensor, uint32_t numPackets, std::shared_ptr<CAttributeProvider> attrProvider)
...
SIPLStatus CPoolManager::Init(bool enableLateAttach)
{
    LOG_DBG("Pool Init.\r\n");
    ...
    LOG_DBG("Pool: Consumer count is %u\n", m_numConsumers);
...
SIPLStatus CPoolManager::HandlePoolBufferSetup(void)
{
    ...
    for (uint32_t i = 0U; i < m_numProdElem; ++i) {
        sciErr = NvSciStreamBlockElementAttrGet(m_handle, NvSciStreamBlockType_Producer, i, &m_prodElems[i].userName,
    ...
    ElemAttr elem[MAX_NUM_ELEMENTS];
    ...
    for (p = 0; p < m_numProdElem; ++p) {
        ...
        NvSciBufAttrList cudaBufAttrList = nullptr;
        if (m_enableLateAttach && poolElem->userName != ELEMENT_TYPE_METADATA) {
            assert(m_attrProvider != nullptr);

            SIPLStatus status = NVSIPL_STATUS_OK;
            status = m_attrProvider->GetBufAttrList(ConsumerType::CUDA_CONSUMER, &cudaBufAttrList);
            CHK_STATUS_AND_RETURN(status, "m_attrProvider->GetBufAttrList.");
            unreconciled.push_back(cudaBufAttrList);
        }
        ...
        if (cudaBufAttrList != nullptr)
        {
            NvSciBufAttrListFree(cudaBufAttrList);
        }
        unreconciled.clear();
...
SIPLStatus CPoolManager::HandlePacketsStatus(void)
{
    ...
    /* Check each packet */
    for (uint32_t p = 0; p < m_numPackets; ++p) {
