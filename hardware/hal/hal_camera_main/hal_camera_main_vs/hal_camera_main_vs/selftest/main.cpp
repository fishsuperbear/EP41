		...
		*/
        ...
        for (blockidx = 0;blockidx<3;blockidx++){
            ...
            for (sensorindex = 0; sensorindex < 4; sensorindex++)
            {
                ...
                printf("RegisterCallback blockindex=%u, sensorindex=%u\r\n", blockidx, sensorindex);
                // see blocktype note of CameraDeviceDataCbRegInfo
                // currently only one sensor type in one block, need to change it in the future
                ...
                if ((ret = _pcameradevice->RegisterDataCallback(&datareginfo, handle_enc_buffer)) < 0) {
...
int main(int argc, char* argv[])
{
	...
	_pcameradevice = ICameraDevice::GetInstance(HAL_CAMERA_VERSION_0_1, true);
